package ListCentral::Admin::Reports;
use strict;

use ListCentral::SETUP;
use ListCentral::Utilities::Date;
use ListCentral::Utilities::Search;
use ListCentral::Admin::Calculator;

##########################################################
# ListCentral::Admin::Reports 
##########################################################

=head1 NAME

   ListCentral::Admin::Reports.pm

=head1 SYNOPSIS

   $ListReports = new ListCentral::Admin::Reports($dbh, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Reports constructor");

   if(!$self->{DBManager}){
      die $self->{Debugger}->log("Where is ListCentral::Admin::Reports's DBManager??");
   }
   if(!$self->{Debugger}){
      print STDERR "No debugger? $self->{Debugger}\n";
   }

   $self->{Calculator} = new ListCentral::Admin::Calculator("Debugger" => $self->{Debugger}, "DBManager" => $self->{DBManager},
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Reports->getReport with page $report");

   if($report eq "UserSettings"){
      return $self->getUserSettingsReport($self->{cgi}->{UserID});
   }
   my $page = "$ListCentral::SETUP::ADMIN_DIR_PATH/reports/$report.html";
   my $content = "";
   my $template = $page;
   ...
   foreach my $line (@lines) {
      if($line =~ m/<!--(.+)-->/){
         ...
         if($line =~ m/<!--URL-->/){
            ...
            my $TopSearches = $self->getTopSearches($ListCentral::SETUP::SEARCHES_REPORT_LIMIT);
            $line =~ s/<!--TopSearches-->/$TopSearches/;
         }elsif($line =~ m/<!--RecentSearches-->/){
            my $RecentSearches = $self->getRecentSearches($ListCentral::SETUP::SEARCHES_REPORT_LIMIT);
...
#############################################
   ...
   my $TopReferrers = $ReferrerObj->getTopReferrers($ListCentral::SETUP::TOP_REFERRERS_REPORT_LIMIT);
...
#############################################
   ...
   my $TopSearches = $SearchHistoryObj->getTopSearches($ListCentral::SETUP::SEARCHES_REPORT_LIMIT);
...
#############################################
   ...
   my $SearchHistoryObj = $self->{DBManager}->getTableObj("SearchHistory");
   my $RecentSearches = $SearchHistoryObj->getRecentSearches($ListCentral::SETUP::SEARCHES_REPORT_LIMIT);

   my $Date;
   my $RecentSearchesRows = "";
   foreach my $ID(sort{$RecentSearches->{$b}->{Date} <=> $RecentSearches->{$a}->{Date}} keys %{$RecentSearches}) {
      $Date = ListCentral::Utilities::Date::getShortHumanFriendlyDate($RecentSearches->{$ID}->{Date});
...
#############################################
   ...
   foreach my $ID(sort keys %{$Complaints}) {
      ...
      my $Date = ListCentral::Utilities::Date::getHumanFriendlyDate($Complaints->{$ID}->{CreateDateFormatted});
...
#############################################
   ...
   foreach my $ID(sort{$PopularLists->{$b}->{'24Hours'} <=> $PopularLists->{$a}->{'24Hours'}} keys %{$PopularLists}) {
      ...
      $PopularListRows .= qq~<tr><td class="StatsTableData">$count</td>
                                 <td class="StatsTableData">$ID</td>
                                 <td class="StatsTableData"><a href="$ListCentral::SETUP::URL$PopularLists->{$ID}->{ListURL}">
...
#############################################
   ...
   foreach my $field (sort keys %{$UserSettings}) {
      ...
      if($field =~ m/(\w+)ID$/){
         ...
         $niceField = $field;
         $value = "<img src='$ListCentral::SETUP::URL/$ListCentral::SETUP::USER_CONTENT_PATH/$UserSettings->{$field}' />";
...
#############################################
   ...
   foreach my $ID(sort{$PopularLists->{$b}->{PopularityScore} <=> $PopularLists->{$a}->{PopularityScore}} keys %{$PopularLists}) {
      ...
      $PopularListRows .= qq~<tr><td class="StatsTableData">$count</td>
                                 <td class="StatsTableData">$ID</td>
                                 <td class="StatsTableData"><a href="$ListCentral::SETUP::URL/lists.html?ListID=$ID">
                                     $PopularLists->{$ID}->{Name}</a></td>
