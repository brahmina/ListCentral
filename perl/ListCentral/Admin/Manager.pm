package ListCentral::Admin::Manager;
use strict;

use ListCentral::SETUP;
use ListCentral::DB::DBManager;

use ListCentral::Admin::Reports;
use ListCentral::Admin::FeedbackManager;
use ListCentral::Admin::ThemeManager;
...
use ListCentral::Utilities::EmailManager;
use ListCentral::Utilities::HelpManager;

##########################################################
# ListCentral::Admin::Manager 
##########################################################

=head1 NAME

   ListCentral::Admin::Manager.pm

=head1 SYNOPSIS

   $ListManager = new ListCentral::Admin::Manager($dbh, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager constructor");
   ...
   $self->{Reports} = new ListCentral::Admin::Reports(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
...
#############################################
   ...
   #$self->{Debugger}->debug("in ListCentral::Admin::Manager handleRequest with $file_requested");
   ...
   if($self->{cgi}->{todo}){
      ...
      if($self->{cgi}->{todo} =~ s/ThemeManager\.(\w+)//){
         $todo = $1;
         $self->{ThemeManager} = new ListCentral::Admin::ThemeManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                  DBManager=>$self->{DBManager});
         my $file = $self->{ThemeManager}->$todo();
         $content = $self->printPage($file);
      }elsif($self->{cgi}->{todo} =~ s/FeedbackManager\.(\w+)//){
         $todo = $1;
         $self->{FeedbackManager} = new ListCentral::Admin::FeedbackManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                  DBManager=>$self->{DBManager});
         ...
         $todo = $1;
         $self->{AdServer} = new ListCentral::Utilities::AdServer(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                  DBManager=>$self->{DBManager});
         my $file = $self->{AdServer}->$todo();
         $content = $self->printPage($file);
      }elsif($self->{cgi}->{todo} =~ s/HelpManager\.(\w+)//){
         $todo = $1;
         $self->{HelpManager} = new ListCentral::Utilities::HelpManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                 DBManager=>$self->{DBManager});
         my $file = $self->{HelpManager}->$todo();
         $content = $self->printPage($file);
      }elsif($self->{cgi}->{todo} =~ s/EmailManager\.(\w+)//){
         $todo = $1;
         $self->{EmailManager} = new ListCentral::Utilities::EmailManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                 DBManager=>$self->{DBManager});
   ...
   $self->{Debugger}->debug("leaving ListCentral::Admin::Manager handleRequest");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager->printPage with page $page");
   ...
   foreach my $line (@lines) {
      if($line =~ m/<!--(.+)-->/){
         ...
         if($line =~ m/<!--URL-->/){
            ...
            my $RootPath = $ListCentral::SETUP::ROOT_PATH . "Lists/";
            $line =~ s/<!--RootPath-->/$RootPath/;
         }elsif($line =~ m/<!--Display\.Page\.(.+)-->/){
            # Display.Page.PathttoPageNameFromDocRoot
            my $page = $1;
            my $file = "$ListCentral::SETUP::ADMIN_DIR_PATH/$page";
            ...
            if(!$self->{PageTitle}){
               $self->{PageTitle} = $ListCentral::SETUP::DEFAULT_ADMIN_PAGE_TITLE;
            }
            ...
            if($table eq "User"){
               ...
               if($Feedback->{ProblematicListID}){
                  ...
                  $value = qq~<a href="$ListCentral::SETUP::URL/$ListURL">$List->{Name}</a>~;
               }              
            }            
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager::doLogin");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager::checkLogin ");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager::doLogout ");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::Login");
   ...
   if(!$self->{ThisAdminUser}->{ID}){
      $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/login.html");
   }else{
      $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/index.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::Logout");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager::SearchUsers"); 
   ...
   if($Qid && !$UsersHash->{$Qid}->{ID}){
      ...
      $ResultsRows = $SearchModule->processSearchResults("$ListCentral::SETUP::ADMIN_DIR_PATH/Utilities/Search/user_search_results_rows.html", $UsersHash);
   }
   ...
   my $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/Utilities/Search/user_search_results.html");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Admin::Manager::SearchLists"); 
   ...
   if($self->{cgi}->{Query} ne ""){
      ...
      my $Calculator = new ListCentral::Admin::Calculator("Debugger" => $self->{Debugger}, "DBManager" => $self->{DBManager},
						      "UserManager" => $self->{UserManager});
      my $ListObj = $self->{DBManager}->getTableObj("List");
      my $count = 0;
      ...
      if($count == 0){
         ...
         $ResultsRows = $SearchModule->processSearchResults("$ListCentral::SETUP::ADMIN_DIR_PATH/Utilities/Search/list_search_results_rows.html", $ListsHash);
      }
      ...
      $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/Utilities/Search/list_search_results.html");
   }else{
      $self->{ErrorMessages} .= "Empty Search Query";
      $content = $self->printPage("$ListCentral::SETUP::DIR_PATH/index.html");
   }
...
#############################################
   ...
   if($Mailer->testEmail($toEmail)){
      ...
      $Mailer->send_email($toEmail, $ListCentral::SETUP::FROM_EMAIL, "Message from List Central", $EmailBody);
   }else{
      # Don't send
   ...
   my $content;
   if($ErrorMessage eq ""){
      $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/email_user_success.html");
   }else{
      $self->{ErrorMessages} .= $ErrorMessage;
      $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/email_user.html");
   }
...
#############################################
   ...
   use ListCentral::Admin::Minify;
   my $Minifier = new ListCentral::Admin::Minify(Debugger => $self->{Debugger});
   $Minifier->minifyJS();
   my $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/theme_management.html");
   return $content;
...
#############################################
   ...
   return $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/users.html");
...
#############################################
   ...
   return $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/lists.html");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddFeedbackReply");

   my $FeedbackManager = new ListCentral::Admin::FeedbackManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                  DBManager=>$self->{DBManager});
   ...
   my $content = $self->printPage("$ListCentral::SETUP::ADMIN_DIR_PATH/feedback.html");
