package ListCentral::Utilities::Search;
use strict;

use ListCentral::SETUP;

##########################################################
# ListCentral::Utilities::Search 
##########################################################

=head1 NAME

   ListCentral::Mailer.pm

=head1 SYNOPSIS

   $Mailer = new ListCentral::Utilities::Search(Debugger);

=head1 DESCRIPTION

Used to create, manage, and send emails for the Lists system.

=head2 ListCentral::Utilities::Search Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Mailer constructor");
...
#############################################
   ...
   my $Limit = $ListCentral::SETUP::CONSTANTS{'DEFAULT_LIST_SEARCH_RESULTS_PER_PAGE'};

   $self->{Debugger}->debug("in ListCentral::Utilities::Search::doBasicSearch with $Query, $Page, $IncludeUsersPrivateLists");
   ...
   if($AndWhereClauseList ne "" && $count < $ListCentral::SETUP::CONSTANTS{'DEFAULT_LIST_SEARCH_RESULTS_PER_PAGE'}){
   ...
   if($count < $ListCentral::SETUP::CONSTANTS{'DEFAULT_LIST_SEARCH_RESULTS_PER_PAGE'} && !$OneWordQuery){
   ...
   if($count < $ListCentral::SETUP::CONSTANTS{'DEFAULT_LIST_SEARCH_RESULTS_PER_PAGE'}){
      # Full list item match
      my $sql = "SELECT List.*
                 FROM $ListCentral::SETUP::DB_NAME.ListItem, $ListCentral::SETUP::DB_NAME.List, $ListCentral::SETUP::DB_NAME.ListPoints, $ListCentral::SETUP::DB_NAME.User
                 WHERE ($AndWhereClauseListItem) AND ListItemStatusID != 0 AND (Public = 1 $LoggedInUserClause) 
                        AND List.Status = 1 AND $ListCentral::SETUP::DB_NAME.ListItem.ListID = $ListCentral::SETUP::DB_NAME.List.ID AND $ListCentral::SETUP::DB_NAME.List.ID = $ListCentral::SETUP::DB_NAME.ListPoints.ListID
                        AND List.UserID = User.ID AND User.Status > 0
                 ORDER BY RankingReddit DESC, CreateDate DESC
                 LIMIT $Limit";
      $count += $self->runSQLSelect($sql, $count, \%results);
...
#############################################
   ...
   #  Need a number of items per page variable
   my $Limit = $ListCentral::SETUP::CONTSTANTS{'DEFAULT_USER_SEARCH_RESULTS_PER_PAGE'};

   $self->{Debugger}->debug("in ListCentral::Utilities::Search::doBasicSearch with $Query");
   ...
   my $count = 0;
   my $sql = "SELECT *
              FROM $ListCentral::SETUP::DB_NAME.User 
   ...
   foreach my $ID(keys %results) {
      $results{$ID}->{FormattedCreateDate} = ListCentral::Utilities::Date::getHumanFriendlyDate{$results{$ID}->{CreateDate}};
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::Search::processSearchResults with $TemplateRows, $Limit");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::Search::getPagenation with $page, $pageCount");
