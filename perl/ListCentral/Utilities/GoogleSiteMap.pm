package ListCentral::Utilities::GoogleSiteMap;
use strict;
...
##########################################################
# ListCentral::GoogleSiteMap 
##########################################################

=head1 NAME

   ListCentral::Utilities::GoogleSiteMap.pm

=head1 SYNOPSIS

   $GoogleSiteMap = new ListCentral::Utilities::GoogleSiteMap(Debugger);

=head1 DESCRIPTION

Used to place google analytic tracting information on the site pages

=head2 ListCentral::Utilities::GoogleSiteMap Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::GoogleSiteMap constructor");
...
#############################################
   ...
   my $command = "rm $ListCentral::SETUP::DIR_PATH/*.gz";
   my $output = `$command`;

   my $mapCount = 1;
   my $sitemap = $ListCentral::SETUP::GOOGLE_SITE_MAP . $mapCount . ".gz";
   $mapCount++;
   ...
   $map->add({
    loc        => "$ListCentral::SETUP::URL/$_.html",
    changefreq => 'weekly',
   ...
   foreach my $ID(sort keys %{$Lists}) {
      if($count >= 50000){
         ...
         $sitemap = $ListCentral::SETUP::GOOGLE_SITE_MAP . $mapCount . ".gz";
         $map = WWW::Google::SiteMap->new(file => "$ListCentral::SETUP::GOOGLE_SITE_MAP");
         $mapCount++;
         $count = 1;
      }
      ...
      my $url = $ListCentral::SETUP::URL . $ListObj->getListURL($Lists->{$ID}, $ListGroup);
      $map->add({
          loc        => $url,
          changefreq => 'daily',
          priority   => 0.8, 
