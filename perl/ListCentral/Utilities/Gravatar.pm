package ListCentral::Utilities::Gravatar;
use strict;
...
##########################################################
# ListCentral::Gravatar 
##########################################################

=head1 NAME

   ListCentral::Utilities::Gravatar.pm

=head1 SYNOPSIS

   $Gravatar = new ListCentral::Utilities::Gravatar(Debugger);

=head1 DESCRIPTION

Used to communicate with Gravatar for getting and setting gravatars used on List Central

=head2 ListCentral::Utilities::Gravatar Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::Gravatar constructor");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::Gravatar::setGravatarEmail with $email, $UserID");
...
#############################################
   ...
   $self->{Debugger}->debug("In ListCentral::Gravatar::getGravatarURL with $UserID and $GravatarEmail");
   ...
   my $GravatarURL = "";
   if($GravatarEmail){
      my $default = $ListCentral::SETUP::GRAVATAR_DEFAULT_IMAGE;
      my $size = $ListCentral::SETUP::GRAVATAR_DEFAULT_SIZE;
      ...
      $GravatarURL = $ListCentral::SETUP::GRAVATAR_DEFAULT_IMAGE;
   }
