...
package ListCentral::Utilities::AmazonRequestSignatureHelper;
...
sub new {
    ...
    debug ("instantiating class \"$classname\"");
    
    debug("self->{AWSAccessKeyId}: $self->{AWSAccessKeyId}\n");
    
    die 'Need AWSAccessKeyId argument' unless exists $self->{AWSAccessKeyId};
    die 'Need AWSSecretKey argument' unless exists $self->{AWSSecretKey};
    die 'Need EndPoint argument' unless exists $self->{EndPoint};
    

    # end-point must be in lowercase
    $self->{EndPoint} = lc($self->{EndPoint});
    ...
    debug ("constructed \"$classname\" instance: " . Dumper($self));
    return $self;
}
...
sub digest {
    my ($self, $x) = @_;
    my $digest = hmac_sha256_base64 ($x, $self->{AWSSecretKey});
