package ListCentral::Utilities::EmailManager;
use strict;
...
##########################################################
# ListCentral::EmailManager 
##########################################################

=head1 NAME

   ListCentral::Utilities::EmailManager.pm

=head1 SYNOPSIS

   $EmailManager = new ListCentral::Utilities::EmailManager(Debugger);

=head1 DESCRIPTION

Used to get resources from the web

=head2 ListCentral::Utilities::EmailManager Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::EmailManager constructor");
...
#############################################
   ...
   return "$ListCentral::SETUP::ADMIN_DIR_PATH/email_management.html";
}
...
#############################################
   ...
   $self->{Debugger}->debugNow("in ListCentral::Utilities::Email->TestEmail");
   ...
   if($EmailID){
      ...
      if($Email->{ID}){
         ...
         foreach my $toEmail(@ListCentral::SETUP::TEST_EMAILS) {
            ...
            my $EmailText = $PageGetter->getBasicPage("$ListCentral::SETUP::DIR_PATH/emails/email_template.txt", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debugNow("in ListCentral::Utilities::Email->SendEmail");
...
#############################################
   ...
   $self->{Debugger}->debugNow("in ListCentral::Utilities::Email->SendEmailToList");
...
#############################################
   ...
   $self->{Debugger}->debugNow("in ListCentral::EmailManager::GetEmailListingsAdmin"); 
   ...
   foreach my $ID(sort{$Emails->{$a}->{Ordering} <=> $Emails->{$b}->{Ordering}} keys %{$Emails}) {
      ...
      $Listings .= $PageGetter->getBasicPage("$ListCentral::SETUP::ADMIN_DIR_PATH/edit_email.html", \%Data);
