package ListCentral::Utilities::BlogInteraction;
use strict;
...
##########################################################
# ListCentral::BlogInteraction 
##########################################################

=head1 NAME

   ListCentral::Utilities::BlogInteraction.pm

=head1 SYNOPSIS

   ListCentral::Utilities::BlogInteraction::function(Debugger);
...
#############################################
   ...
   #print STDERR "in ListCentral::Utilitites::BlogInteraction::writeAboutBlogTeaser\n"; 

   my $aboutTeaserFile = "$ListCentral::SETUP::DIR_PATH/about/about_blog_teaser.html";
   my $blogTeaser = getBlogTeaser($ListCentral::SETUP::BLOG_TEASER_NUMBER_OF_POSTS, $DBManager, $UserManager);
...
#############################################
   ...
   #print SDTERR "in ListCentral::Utilities::BlogInteraction::getBlogTeaser with $numberOfPostsToGet\n";

   use XML::Simple;
   use LWP::Simple;
        
   my $xml = get($ListCentral::SETUP::BLOG_RSS_URL);
   ...
   foreach my $item_hash(@{$response->{channel}->{item}}){
       if($count < $numberOfPostsToGet){
          my $param = "";
          if($item_hash->{'dc:creator'} eq "brahmina"){
...
#############################################
   ...
   #print STDERR "in ListCentral::ListManager::getAvatar with param: $param\n";
   ...
   my $template = "$ListCentral::SETUP::DIR_PATH/widgets/avatar.html";
