package ListCentral::Utilities::ImageResizer;
use strict;
...
##########################################################
# ListCentral::ImageResizer 
##########################################################

=head1 NAME

   ListCentral::Utilities::ImageResizer.pm

=head1 SYNOPSIS

   $ImageResizer = new ListCentral::Utilities::ImageResizer(Debugger);

=head1 DESCRIPTION

Used to place google analytic tracting information on the site pages

=head2 ListCentral::Utilities::ImageResizer Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::ImageResizer constructor");
...
=item B<Parameters :>
   ...
   1. $self - Reference to a ListCentral::Utilities::ImageResizer object
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::ImageResizer::resizeImage with $OriginalImage");
   ...
   if($width > $ListCentral::SETUP::MAX_IMAGE_WIDTH_LARGE || $height > $ListCentral::SETUP::MAX_IMAGE_WIDTH_LARGE){
      ...
      my $LargeGeo = $ListCentral::SETUP::MAX_IMAGE_WIDTH_LARGE . "x" . $ListCentral::SETUP::MAX_IMAGE_WIDTH_LARGE;
      ...
      if($extension =~ m/jpeg/i || $extension =~ m/jpg/i){
         $Error = $ImageMagick->Set(compression => 'JPEG', quality=>$ListCentral::SETUP::IMAGE_QUALITY);
      }else{
         $Error = $ImageMagick->Set(quality=>$ListCentral::SETUP::IMAGE_QUALITY);
      }
   ...
   if($width > $ListCentral::SETUP::MAX_IMAGE_WIDTH_MEDIUM || $height > $ListCentral::SETUP::MAX_IMAGE_WIDTH_MEDIUM){
      my $ImageMagick = Image::Magick->new;
      my $Error = $ImageMagick->Read($OriginalImage);
      ...
      my $MediumGeo = $ListCentral::SETUP::MAX_IMAGE_WIDTH_MEDIUM . "x" . $ListCentral::SETUP::MAX_IMAGE_WIDTH_MEDIUM;
   ...
   if($width > $ListCentral::SETUP::MAX_IMAGE_WIDTH_SMALL || $height > $ListCentral::SETUP::MAX_IMAGE_WIDTH_SMALL){
      my $ImageMagick = Image::Magick->new;
      my $Error = $ImageMagick->Read($OriginalImage);
      ...
      my $SmallGeo = $ListCentral::SETUP::MAX_IMAGE_WIDTH_SMALL . "x" . $ListCentral::SETUP::MAX_IMAGE_WIDTH_SMALL;
   ...
   if($ListCentral::SETUP::DELETE_ORIGINAL_IMAGE){
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::ImageResizer::resizeAvatar with $OriginalImage");
   ...
   if($width > $ListCentral::SETUP::MAX_AVATAR_WIDTH || $height > $ListCentral::SETUP::MAX_AVATAR_WIDTH){
      ...
      if($width >= $height){
         ...
         my $targetWidth = int($width * $ListCentral::SETUP::MAX_AVATAR_WIDTH / $height);
         my $AvatarGeo = $targetWidth . "x" . $ListCentral::SETUP::MAX_AVATAR_WIDTH;
         ...
         my $geo = $ListCentral::SETUP::MAX_AVATAR_WIDTH . "x" . $ListCentral::SETUP::MAX_AVATAR_WIDTH;;
         ...
         my $targetHeight = int($height * $ListCentral::SETUP::MAX_AVATAR_WIDTH / $width);
         my $AvatarGeo = $ListCentral::SETUP::MAX_AVATAR_WIDTH . "x" . $targetHeight;
         $self->{Debugger}->debug("AvatarGeo: $AvatarGeo");
         $Error = $ImageMagick->AdaptiveResize(geometry=>$AvatarGeo, blur => 0);
         $self->{Debugger}->debug("Resize image: $Error");
         if($Error){
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::ImageResizer::resizeAmazonImage with $OriginalImage");
   ...
   if($width > $ListCentral::SETUP::MAX_AMAZON_IMAGE_WIDTH || $height > $ListCentral::SETUP::MAX_AMAZON_IMAGE_WIDTH){
      $self->{Debugger}->debug("in if");
      my $ImageMagick = Image::Magick->new;
      my $Error = $ImageMagick->Read($OriginalImage);
      ...
      my $AmazonGeo = $ListCentral::SETUP::MAX_AMAZON_IMAGE_WIDTH . "x" . $ListCentral::SETUP::MAX_AMAZON_IMAGE_WIDTH;
