package ListCentral::ListsManager;
use strict;

use ListCentral::SETUP;
use ListCentral::DB::DBManager;
use ListCentral::UserManager;
use ListCentral::Editor;

use ListCentral::Utilities::Cache;
use ListCentral::Utilities::Date;
use ListCentral::Utilities::Ranker;
use ListCentral::Utilities::Search;
use ListCentral::Utilities::GoogleAnalytics;

##########################################################
# ListCentral::ListsManager 
##########################################################

=head1 NAME

   ListCentral::ListsManager.pm

=head1 SYNOPSIS

   $ListsManager = new ListCentral::ListsManager($dbh, $cgi, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager constructor");

   if(!$self->{Debugger}){
      print STDERR "No debugger? $self->{Debugger}\n";
      $self->{ErrorMessages} = "No debugger? $self->{Debugger}??";
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
   }
   ...
   $self->{Editor} = new ListCentral::Editor(Debugger => $self->{Debugger}, 
                                       cgi => $self->{cgi}, 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager handleRequest with $file_requested & todo: $self->{cgi}->{todo}");
   ...
   if($ListsDev::SETUP::UNDER_CONSTUCTION){
      ...
      if (!$IPisPermitted) {
         $self->setThemeID();
         my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/under_construction.html");
         return $content;
      }
   }
   ...
   if($self->{cgi}->{todo}){
      ...
      if(!$self->{UserManager}->{ThisUser}->{ID} && !$ListCentral::SETUP::NOT_LOGGED_IN_TODOS{$self->{cgi}->{todo}}){
         ...
         if($self->{cgi}->{ajax}){
            $content =  "<p><b>" . $ListCentral::SETUP::MESSAGES{'NO_PERMISSION'} . "</b></p>";
         }else{
            $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION'};
            $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/error.html");
         }
      }else{
      ...
      my $Cacher = new ListCentral::Utilities::Cache(Debugger => $self->{Debugger}, 
   ...
   $self->{Debugger}->debug("leaving ListCentral::ListsManager handleRequest");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getPage with page $page, ThemeID $self->{ThemeID} Logged in: $self->{UserManager}->{ThisUser}->{ID}");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getPageData with page $page");
   ...
   if($page !~ m/upper_links/ && $page !~ m/login/ && $page !~ m/preloaded_images/){
      ...
      if($self->{cgi}->{UserID}){
         ...
         if(! $self->{PageOwner}->{ID}){
            $self->{ThemeID} = $ListCentral::SETUP::CONSTANTS{'DEFAULT_THEME'};
            $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'DEACTIVATED_USER'};
            $page = "$ListCentral::SETUP::DIR_PATH/error.html";        
         }else{
            if($page =~ m/settings/){
      ...
      if($self->{cgi}->{"ListID"}){        
         ...
         if($self->{UserManager}->{ThisUser}->{ID} == $self->{cgi}->{UserID}){
            if(! $self->{UserManager}->userListCount($self->{UserManager}->{ThisUser}->{ID})){
               $self->{MessageToUser} = $self->processPage("$ListCentral::SETUP::DIR_PATH/loggedin/welcome.html");
            }
         }
         $self->{ListsContent} = $self->getPage("$ListCentral::SETUP::DIR_PATH/users_space.html");
      }
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->doPageMods with page $page");
   ...
   if($self->{cgi}->{"ListID"} && $page =~ m/lists/){
      ...
      if(!$self->{ThisList}->{ID}){
         ...
         $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'DELETED_LIST'};
         $page = "$ListCentral::SETUP::DIR_PATH/error.html";
      }elsif(!$self->{ThisList}->{Public} && $self->{UserManager}->{ThisUser}->{ID} != $self->{ThisList}->{UserID}){
         # Private list
         $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_VIEW_LIST'};
   ...
   if($ListCentral::SETUP::ACCOUNT_ONLY_PAGES{$page} && !$self->{UserManager}->{ThisUser}->{ID}){
      #$self->{Debugger}->throwNotifyingError("Someone trying to get page they shouldn't be: page: $page, self->{cgi}->{UserID}: $self->{cgi}->{UserID}, self->{UserManager}->{ThisUser}->{ID}: $self->{UserManager}->{ThisUser}->{ID}");
      if($self->{cgi}->{ajax}){
         return "<p><b>" . $ListCentral::SETUP::MESSAGES{'NO_PERMISSION'} . "</b></p>";
      }else{
         $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION'};
         $page = "$ListCentral::SETUP::DIR_PATH/error.html";
      }
   ...
   # about page
   if($self->{cgi}->{"UserID"} == $ListCentral::SETUP::ABOUT_USER_ACCOUNT || $page =~ m/about/ || 
         ($page =~ m/users_space/ && $self->{cgi}->{"UserID"} == $ListCentral::SETUP::ABOUT_USER_ACCOUNT)){

      $self->{Debugger}->debug("Dealing with about page business with $page");

      $self->{cgi}->{"UserID"} = $ListCentral::SETUP::ABOUT_USER_ACCOUNT;

      $self->{ListsNavHeader} = "about list central";
      if($self->{ListsNavigation} !~ m/Tag Cloud/){
         my $aboutNavExtra = $self->processPage("$ListCentral::SETUP::DIR_PATH/about/about_nav_addition.html");
         $self->{ListsNavigation} = $aboutNavExtra . $self->{ListsNavigation};
      }      

      if($self->{UserManager}->{ThisUser}->{ID} != $ListCentral::SETUP::ABOUT_USER_ACCOUNT){
         $self->{"DontShowListForm"} = 1;
      }

      if($page =~ m/help/){
         if($page =~ m/help_page/){
            use ListCentral::Utilities::HelpManager;
            my $HelpManager = new ListCentral::Utilities::HelpManager(cgi=>$self->{cgi}, Debugger=>$self->{Debugger}, 
                                                         DBManager=>$self->{DBManager});
            ($self->{"HelpPageContent"}, $self->{PageTitle}) = $HelpManager->getHelpPage($self->{cgi}->{HelpPageID});
            $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/about/help_page.html";
         }else{
            $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/about/help.html";
         }

         
         $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
      }elsif($page =~ m/tagcloud/){
         $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/about/tagcloud.html";
         $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
      }elsif($page =~ m/faq/){
         #/list/about_list_central/frequently_asked_questions/35/193/lists.html
         $self->{cgi}->{"ListID"} = $ListCentral::SETUP::CONSTANTS{'FAQ_LISTID'};
         $self->{ThisList} = $self->{ListObj}->get_by_ID($self->{cgi}->{"ListID"});
         $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
      }elsif($page =~ m/about.html/){
         if(! $self->{AboutBlogTeaser}){
            $self->{AboutBlogTeaser} = $self->processPage("$ListCentral::SETUP::DIR_PATH/about/about_blog_teaser.html");
         }  

         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/about/about.html";
         $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
      }
   }
   ...
   if(($page =~ m/user_space/ || $page =~ m/settings/ || $page =~ m/create_and_edit/ || $page =~ m/messages/) && !$self->{cgi}->{ajax}){
      ...
      if($page =~ m/create_and_edit/ && !$self->{cgi}->{todo}){
         ...
         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/create_and_edit.html";
         ...
         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/settings.html";;
      }elsif($page =~ m/messages/ && !$self->{cgi}->{todo}){
         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/messages.html";;
      }else{
         $self->{"DisplaySomethingOtherThanList"} = $page;
      }
      $page = "$ListCentral::SETUP::DIR_PATH/lists.html";
   ...
   if($self->{UserManager}->{ThisUser}->{ID}){
      ...
      if($self->{UserManager}->{ThisUser}->{ID} == $self->{cgi}->{UserID}){
         if(-e "$ListCentral::SETUP::DIR_PATH/owner/$page"){      
            $page = "$ListCentral::SETUP::DIR_PATH/owner/$page";
         }elsif(-e "$ListCentral::SETUP::DIR_PATH/loggedin/$page"){
...
#############################################
   ...
   if($self->{BasicFileCache}->{$page}){
      ...
      if(! open (PAGE, $template)){
         ...
         return $self->processPage("$ListCentral::SETUP::DIR_PATH/errors/404.html");
      }
      @lines = <PAGE>;
      close PAGE;
   ...
   foreach my $line (@lines) {
      if($line =~ m/<!--(.+)-->/){
         ...
         if($line =~ m/<!--URL-->/){
            ...
            my $page = $1;
            my $file = "$ListCentral::SETUP::DIR_PATH/$page";
            ...
            use ListCentral::Utilities::Fail;
            my $Fail = new ListCentral::Utilities::Fail(Debugger=>$self->{Debugger});
            ...
            if($ID){
               ...
               if($field =~ m/Date/){
                  my $dateFormatted = ListCentral::Utilities::Date::getHumanFriendlyDate($value);
            ...
            # Display.Page.PathttoPageNameFromDocRoot
            my $page = $1;
            my $file = "$ListCentral::SETUP::DIR_PATH/listpieces/$page";
            ...
            use ListCentral::Utilities::GoogleAnalytics;
            my $code = ListCentral::Utilities::GoogleAnalytics::getCode();
            $line =~ s/<!--GoogleAnalytics-->/$code/;
         }elsif($line =~ m/<!--CurrentDate-->/){
            my $date = ListCentral::Utilities::Date::getFullHumanFriendlyDate(time());
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddList");

   if($self->{cgi}->{"List.Name"} eq ""){
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'BLANK_LIST_NAME'};   
      $self->{"DisplayCreateListForm"} = 1;
      $self->{CreateEditTab}->{CreateList} = 1;
      my $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/owner/create_and_edit.html");
      return $content;
   }

   $self->{cgi}->{"List.Name"} = ListCentral::Utilities::StringFormator::htmlEncode($self->{cgi}->{"List.Name"});
   ...
   if($ListID =~ m/\D+/){
      ...
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/lists.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddListItem");
   ...
   if(! $self->{cgi}->{"ListID"}){
      ...
      $self->{ErrorMessagesALI} = $ListCentral::SETUP::MESSAGES{'MISC'};
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
   }
   
   $self->{cgi}->{"ListItem.Name"} = ListCentral::Utilities::StringFormator::htmlEncode($self->{cgi}->{"ListItem.Name"});
   ...
   if(! $self->{UserManager}->{ThisUser}->{ID} == $List->{UserID}){
      ...
      $self->{ErrorMessagesALI} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_EDIT_LIST'};
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
   }

   my $content = "";
   if(!$self->{cgi}->{"ListItem.Name"}){
      $self->{ErrorMessagesALI} = $ListCentral::SETUP::MESSAGES{'BLANK_LIST_ITEM'};
   ...
   if($self->{ErrorMessagesALI} eq ""){
      ...
      if($ListItemCount >= $ListCentral::SETUP::CONSTANTS{'LIST_ITEM_LIMIT'}){
         $self->{ErrorMessagesALI} = $ListCentral::SETUP::MESSAGES{'AT_LIST_ITEM_LIMIT'};
      }else{
         # Save the List Item!
         ...
         if($self->{cgi}->{"ListItem.ASIN"}){
            # Create Amazon link 
            my $Amazon = new ListCentral::Utilities::Amazon(Debugger => $self->{Debugger}, DBManager => $self->{DBManager}, 
                                                   UserManager => $self->{UserManager});
   ...
   if($self->{cgi}->{ajax}){
      ...
      return $self->getPage("$ListCentral::SETUP::DIR_PATH/lists.html");
   }
}
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddFeedback");

   use Net::Akismet;
   my $spamchecker = 1;
   my $akismet = Net::Akismet->new(
                        KEY => '5248d5060124',
                        URL => $ListCentral::SETUP::URL,
                ) or $spamchecker = 0;
   ...
   if($FeedbackID =~ m/\D+/){
      ...
      my $content;
      if($self->{cgi}->{ajax}){
         $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/thankyou.html");
      }else{
         $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/thankyou.html");
      }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddListTag");
   ...
   if(! $TagName){
      $self->{ErrorMessagesAT} .= $ListCentral::SETUP::MESSAGES{'BLANK_TAG'};
      ...
      $self->{ErrorMessagesAT} .= $ListCentral::SETUP::MESSAGES{'DISALLOWED_CHARACTERS'};
      my ($TagHTML, $TagCount, $Tags) = $TagObj->getListTags($ListID, $self->{DBManager}, $self->{cgi}->{ajax});
      return "<p class='error'>$self->{ErrorMessagesAT}</p><br />" . $TagHTML;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddListComment");

   # This Error message isn't going to be displayed with the current set up
   if(! $self->{cgi}->{'Comment'}){
      $self->{"ErrorMessagesAC"} .= $ListCentral::SETUP::MESSAGES{'BLANK_COMMENT'};
      my $Comments = $self->getListComments($self->{cgi}->{'ListID'});
      return "<p class='error'>$self->{'ErrorMessagesAC'}</p><br /><br />$Comments";
   }

   if(! $self->{UserManager}->{ThisUser}->{ID}){
      $self->{"ErrorMessagesAC"} .= $ListCentral::SETUP::MESSAGES{'NO_ONE_LOGGED_IN_COMMENT'};
      my $Comments = $self->getListComments($self->{cgi}->{'ListID'});
      return "<p class='error'>$self->{'ErrorMessagesAC'}</p><br /><br />$Comments";
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddListGroup");
   ...
   $self->{"CreateEditTab"}->{"CreateList"} = 1;
   $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/create_and_edit.html";
   my $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/lists.html");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddNotifyOfAlphaRelease");
   ...
   if($AlphaNotifyID =~ m/\D+/){
      ...
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
   }else{
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/notify_thanks.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddStatusSet");
   ...
   if(!scalar(@Statuses)){
      ...
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'INVALID_STATUS_SET'};   
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->AddUser");
   ...
   if($ErrorMessage eq "BETA"){
      $ErrorMessage = "";
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/notify.html");
   }elsif($ErrorMessage ne ""){
      $self->{SignUpErrorMessages} = $ErrorMessage;
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/signup.html");
   }else{
      $self->{cgi}->{UserID} = $self->{UserManager}->{ThisUser}->{ID};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->DeactivateAccount, $self->{UserManager}->{ThisUser}->{ID} logged in");
   ...
   if($self->{UserManager}->{ThisUser}->{ID}){
      ...
      if($hashedCGIPassword eq $hashedPassword){
         ...
         $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/deactivate.html");
      }else{
         $self->{ErrorMessagesEBI} = $ListCentral::SETUP::MESSAGES{'FAIL_PASSWORD_EDIT_ACCOUNT'};
         $self->{cgi}->{UserID} = $self->{UserManager}->{ThisUser}->{ID};
         $self->setThemeID();
         $self->{SettingsTab}->{BasicInfo} = 1;
         $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/settings.html";
      ...
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/index.html");
   }   
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteList Logged in user: $self->{UserManager}->{ThisUser}->{ID}");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteListGroup");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $OwnerOfListGroup){
      ...
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/owner/create_and_edit.html");
   }else{
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_DELETE_GROUP'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteListItem");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $OwnerOfListItem){
      ...
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_DELETE_ITEM'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteListTags");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteStatusSet");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $OwnerOfStatusSet){
      ...
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_DELETE_STATUS_SET'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteBoardPost");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $OwnerOfBoard || $self->{UserManager}->{ThisUser}->{ID} == $OwnerOfBoardPost){
      ...
      $self->setThemeID();
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/users_space.html");
   }else{
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_DELETE_BOARD_POST'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::DeleteComment");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $OwnerOfComment || $self->{UserManager}->{ThisUser}->{ID} == $OwnerOfList){
     ...
     $content = $self->getListComments($List->{ID});
      ...
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_DELETE_COMMENT'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditAvatar");
   ...
   $self->{cgi}->{UserID} = $self->{UserManager}->{ThisUser}->{ID};
   $self->setThemeID();
   my $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/owner/settings.html");
   return $content;
}
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditAboutMe");

   # No one logged in, shouldn't be editing anything!
   if(! $self->{UserManager}->{ThisUser}->{ID}){
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_EDIT_ACCOUNT'};
      $self->{Debugger}->throwNotifyingError("User tried to edit account, but no one logged in");
      my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
      return $content;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditBasicInfo");
   ...
   if($encryptedPassword ne $self->{UserManager}->{ThisUser}->{Password}){
      ...
      if($self->{cgi}->{"User.Password1"} ne ""){
         ...
         if($self->{cgi}->{"User.Password1"} ne $self->{cgi}->{"User.Password2"}){
            $self->{ErrorMessagesEBI} = $ListCentral::SETUP::MESSAGES{'PASSWORD_NONMATCH'};
         }else{
      ...
      if($self->{cgi}->{"User.Email"} ne $self->{UserManager}->{ThisUser}->{Email}){
         ...
         if(! $Mailer->emailValid($self->{cgi}->{"User.Email"})){
            $self->{ErrorMessagesEBI} .= $ListCentral::SETUP::MESSAGES{'INVALID_EMAIL'} . "<br />";
         }else{
            my $Users = $UserObj->get_with_restraints("Email = \"$self->{cgi}->{'User.Email'}\"");
            foreach (keys %{$Users}) {
               if($Users->{$_}){
                  $self->{ErrorMessagesEBI} .= $ListCentral::SETUP::MESSAGES{'DUPLICATE_EMAIL'} . "</br>";
               }  
            }
      ...
      if($self->{cgi}->{"User.Name"} ne $self->{UserManager}->{ThisUser}->{Name}){
         ...
         if($self->{cgi}->{"User.Name"} eq ""){
            $self->{ErrorMessagesEBI} .= $ListCentral::SETUP::MESSAGES{'BLANK_NAME'} . "<br />";
         }else{
      ...
      if($self->{cgi}->{"User.Username"} ne $self->{UserManager}->{ThisUser}->{Username}){
         ...
         if($self->{cgi}->{"User.Username"} eq ""){   
            $self->{ErrorMessagesEBI} .= $ListCentral::SETUP::MESSAGES{'BLANK_USERNAME'} . "<br />"; 
         }else{
            my $Users = $UserObj->get_with_restraints("Username = \"$self->{cgi}->{'User.Username'}\"");
            foreach (keys %{$Users}) {
               if($Users->{$_}){
                  $self->{ErrorMessagesEBI} .= $ListCentral::SETUP::MESSAGES{'DUPLICATE_USERNAME'} . "<br />"; 
               }  
            }
            if($self->{ErrorMessagesEBI} eq ""){
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditTheme");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditEmailing");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditPrivacy");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditListDetails");
   ...
   if($List->{UserID} != $self->{UserManager}->{ThisUser}->{ID}){
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_EDIT_LIST'};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditListItem");
   ...
   if(! $self->{UserManager}->{ThisUser}->{ID} || $self->{UserManager}->{ThisUser}->{ID} != $List->{UserID}){
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_EDIT_LIST_ITEM'};
   ...
   if($EditedListItem{Name} ne $ListItem->{Name}){
      if($EditedListItem{Name} ne ""){
         ...
         $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'EMPTY_LIST_NAME'};
      }
   }
   ...
   if($EditedListItem{Date}){
      ...
      if($ListItem->{DateID}){         
         ...
         if($EditedListItem{Date} ne $Date->{DateEng}){
            ...
            my $epoch = ListCentral::Utilities::Date::getEpochDateTime($EditedListItem{Date});
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->EditListItemOrder");
...
#############################################
   ...
   if($Mailer->emailValid($self->{cgi}->{ToEmail})){
      if($List->{Public} || ($self->{UserManager}->{ThisUser}->{ID} == $List->{UserID})){
         ...
         $Mailer->sendEmail($self->{cgi}->{ToEmail}, 
                            $ListCentral::SETUP::MAIL_FROM_LISTS, 
         ...
         # don't send
         $ErrorMessage = $ListCentral::SETUP::MESSAGES{'NO_PERMISSION_VIEW_LIST'};
      }      
   }else{
      # Don't send
      $ErrorMessage = $ListCentral::SETUP::MESSAGES{'INVALID_EMAIL'};
   }
   ...
   my $content;
   if($ErrorMessage eq ""){
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/email_list_success.html");
   }else{
      $self->{ErrorMessages} .= $ErrorMessage;
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/email_list.html");
   }
   return $content;
}
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::GetEditGroupsForm");
   ...
   my $createEdit = "";
   if($self->{cgi}->{ajax}){
      $createEdit =  $self->getPage("$ListCentral::SETUP::DIR_PATH/owner/create_and_edit.html");
   }else{
      $self->{"DisplayCreateEditForm"} = 1;
      $createEdit =  $self->getPage("$ListCentral::SETUP::DIR_PATH/lists.html");
   }
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::GetEditListItemForm");
   ...
   my $editListItemForm =  $self->processPage("$ListCentral::SETUP::DIR_PATH/owner/listpieces/list_item_edit_form.html", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::getEditListFormAdditions with $ListItem->{Name}");
   ...
   if($ListItem->{LinkID}){
      ...
      $Data{"action"} = "Edit";
      $Data{"Link"} = $Link->{Link};
      $ListItemAdditions .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_link.html", \%Data);
   ...
   if($ListItem->{Description} ne ""){
      ...
      $ListItemAdditions .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_description.html", \%Data);
   ...
   if($ListItem->{DateID}){
      ...
      $ListItemAdditions .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_date.html", \%Data);
   ...
   if($ListItem->{AmazonID}){
      ...
      $ListItemFormExtra .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_extra_element.html", \%Data);
      $ListItemFormExtraHiddenFields .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_extra_hidden_fields.html", \%Data);
      ...
      my $path = $self->{UserManager}->getUsersDirectory($self->{UserManager}->{ThisUser}->{ID});
      $Data{"ExtraImage"} = $ListCentral::SETUP::USER_CONTENT_DIRECTORY . "/" . $path . "/" . $Image->{ID} . "M." . $Image->{Extension};
      $Data{"ExtraLink"} = $ListCentral::SETUP::USER_CONTENT_DIRECTORY . "/" . $path . "/" . $Image->{ID} . "L." . $Image->{Extension};
      ...
      $ListItemAdditions .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_embed.html", \%Data);
      $ListItemFormExtraHiddenFields .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_extra_hidden_fields.html", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::GetHelpListings");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::GetRegionsSelect");
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->GetSettings");
   ...
   my $settings = "";
   if($self->{cgi}->{ajax}){
      $settings = $self->getPage("$ListCentral::SETUP::DIR_PATH/owner/settings.html");
   }else{
      $self->{"DisplaySomethingOtherThanList"} = "$ListCentral::SETUP::DIR_PATH/owner/settings.html";
      $settings = $self->getPage("$ListCentral::SETUP::DIR_PATH/lists.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::Login");

   my $content = "";
   my $ID;
   ($ID, $self->{LoginErrorMessages}) = $self->{UserManager}->doLogin();
   if($ID == -1){
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/deactivated_account.html");
   }else{
      $self->{cgi}->{UserID} = $self->{UserManager}->{ThisUser}->{ID};
      if($self->{UserManager}->{ThisUser}->{ID}){
         $self->setThemeID();
         ...
         $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/login.html");
      }
   }
   return $content;
}
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::Logout");

   $self->{UserManager}->doLogout();
   $self->getPage("$ListCentral::SETUP::DIR_PATH/index.html");
}
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::PostOnBoard: ". $self->{cgi}->{'Board.Message'});

   if($self->{cgi}->{"Board.Message"} eq "" || $self->{cgi}->{"Board.Message"}  eq "Write Something..."){
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'BLANK_BOARD_MESSAGE'};   
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::PublishList");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::SearchLists"); 

   my $content = "";
   if($self->{cgi}->{Query} ne ""){      
      my $SearchModule = new ListCentral::Utilities::Search('Debugger'=>$self->{Debugger}, 'DBManager'=>$self->{DBManager}, 
      ...
      my $pageCount = int($SearchResultsCount / $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'});
      if($SearchResultsCount % $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'}){
         $pageCount++;
      }
      ...
      my $ResultsRows = $SearchModule->processSearchResults("$ListCentral::SETUP::DIR_PATH/widgets/listing_row.html", $ListsHash);
      ...
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/search.html");
   }else{
      $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'BLANK_SERACH_QUERY'};
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/index.html");
   }
...
#############################################
   ...
   my $ResultsRows = $SearchModule->processSearchResults("$ListCentral::SETUP::DIR_PATH/user_search_rows.html", $ListsHash);

   $self->{SearchResultsRows} = $ResultsRows;
   $self->{Pagenation} = "Pages";

   my $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/user_search.html");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::SendForgotPassword");
   ...
   if($User->{ID}){
      ...
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/sentforgotpassword.html");
   }else{
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_EMAIL_FOUND'} . $self->{cgi}->{Email}. '<br />';
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/forgotpassword.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::UploadImage"); 
   ...
   if($filename){
      ...
      foreach my $ext(@ListCentral::SETUP::PERMITTED_IMAGE_EXTENSIONS) {
         if($extension eq $ext){
            $imageTypePermitted = 1;
         }
      }
      if(! $imageTypePermitted){
         $self->{ErrorMessages} .= $ListCentral::SETUP::MESSAGES{'NOT_PERMITTED_IMAGE_TYPE'};
         my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/upload_image.html");
         return $content;
      }    
      ...
      my $userfile = "$ListCentral::SETUP::USER_CONTENT_DIRECTORY/$directory/$ImageID.$extension";
      ...
      # Rezize image
      use ListCentral::Utilities::ImageResizer;
      my $ImageResizer = new ListCentral::Utilities::ImageResizer(Debugger => $self->{Debugger});
      ...
      my $userContentDir = $ListCentral::SETUP::USER_CONTENT_DIRECTORY;
      $userContentDir =~ s/$ListCentral::SETUP::DIR_PATH\///;

      $self->{Debugger}->debug("should have stored image in DB"); 

      my $userContentDirectory = $ListCentral::SETUP::USER_CONTENT_DIRECTORY;
      $userContentDirectory =~ s/$ListCentral::SETUP::DIR_PATH//;
      ...
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/upload_image_success.html");
   }else{
      $self->{"ErrorMessages"} = $ListCentral::SETUP::MESSAGES{'IMAGE_UPLOAD_FAIL'};
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/upload_image.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListNavigation fore UserID $self->{cgi}->{UserID}");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->RetryAmazon");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->GetAmazonChoices");
   ...
   if($self->{cgi}->{"ListItem.Name"} eq ""){
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/Amazon/empty_list_item.html");
   }
   ...
   use ListCentral::Utilities::Amazon;
   my $AmazonObj = new ListCentral::Utilities::Amazon(Debugger => $self->{Debugger}, cgi => $self->{cgi}, 
                                 DBManager => $self->{DBManager}, UserManager => $self->{UserManager});
   $self->{cgi}->{"ListItem.NameDecoded"} = ListCentral::Utilities::StringFormator::htmlDecode($self->{cgi}->{"ListItem.Name"});
   ...
   if($self->{AmazonChoices} =~ m/^Error/){
      ...
      $Content = $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/Amazon/no_choices.html");
   }else{
      $Content = $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/Amazon/choices.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->GetCCImageChoices");
   ...
   if($self->{cgi}->{"ListItem.Name"} eq ""){
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/CCImage/empty_list_item.html");
   }
   
   use ListCentral::Utilities::CCImage;
   my $CCImageObj = new ListCentral::Utilities::CCImage(Debugger => $self->{Debugger}, cgi => $self->{cgi}, DBManager => $self->{DBManager});
   ...
   if($self->{CCImageChoices} =~ m/^500/ || $self->{CCImageChoices} =~ m/^Error/){
      ...
      $Content = $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/CCImage/no_choices.html");
   }else{
      $Content = $self->processPage("$ListCentral::SETUP::DIR_PATH/Utilities/CCImage/choices.html");
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListHTML with $ListID");

   # Gracefilly handle no ListID
   if(! $ListID){
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_LIST_ID'};
   ...
   if($List->{ID} == $ListCentral::SETUP::CONSTANTS{'FAQ_LISTID'}){
   ...
   my $content = "";
   my $template = "$ListCentral::SETUP::DIR_PATH/listpieces/list.html";
   if($ListItemCount == 0){
      $template = "$ListCentral::SETUP::DIR_PATH/listpieces/emptylist.html";
   }
   if($self->{UserManager}->{ThisUser}->{ID} == $List->{UserID}){
      $template =~ s/$ListCentral::SETUP::DIR_PATH/$ListCentral::SETUP::DIR_PATH\/owner/;
   }elsif($self->{UserManager}->{ThisUser}->{ID} && -e "$ListCentral::SETUP::DIR_PATH/loggedin/listpieces/emptylist.html"){
      $template =~ s/$ListCentral::SETUP::DIR_PATH/$ListCentral::SETUP::DIR_PATH\/loggedin/;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::getListItemExtra with $ListItem->{Name}");
   my $template;
   ...
   if($ListItem->{"AmazonID"}){
      ...
      if($width > $ListCentral::SETUP::CONSTANTS{'EMBED_MAX_WIDTH'}){
         my $newWidth = $ListCentral::SETUP::CONSTANTS{'EMBED_MAX_WIDTH'};
   ...
   while($template =~ m/<!--SETUP\.([\w_]+)-->/g){
      my $variable = $1;
      $template =~ s/<!--SETUP\.$variable-->/$ListCentral::SETUP::CONSTANTS{$variable}/g;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListTemplateHash $piece $self->{TemplateHash}");

   if($self->{TemplateHash}->{$piece}){
      return $self->{TemplateHash}->{$piece};
   }else{
      # open listpieces dir
      my $ListPiecesDir = "$ListCentral::SETUP::DIR_PATH/listpieces";
      ...
      foreach my $file(@files) {
         if($piece eq "list_item"){
            if($file =~ m/list_item_([\w_]+)\.html/){
               my $filename = "$ListCentral::SETUP::DIR_PATH/listpieces/$file";
               my $template_type = $1;
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListElement $page and LIST: $List");
      ...
      while($line =~ m/<!--/g){
         if($line =~ m/<!--List\.(\w+)-->/){
            ...
            if($element eq "CreateDateFormatted"){
               $value = ListCentral::Utilities::Date::getShortHumanFriendlyDate($List->{CreateDate});
            }else{
               $value = $List->{$element};
            }
         ...
         if($line =~ m/<!--SETUP\.(\w+)-->/){
            my $field = $1;
            $line =~ s/<!--SETUP\.$field-->/$ListCentral::SETUP::CONSTANTS{$field}/;
         }
      }
      $content .= $line;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListItemHTML with $type, Item: $ListItem->{Name}");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListEmail with $ListID");

   my $List = $self->{ListObj}->get_by_ID($ListID);
   my $ListGroup = $self->{ListGroupObj}->get_by_ID($List->{ListGroupID});
   my $ListURL =  $ListCentral::SETUP::URL . $self->{ListObj}->getListURL($List, $ListGroup->{Name});
   my $ListItemStatuses = $self->{ListItemStatusObj}->get_with_restraints("StatusSet = $List->{StatusSet}");
   $self->{ListName} = $List->{Name};
   ...
   foreach my $ListItemID(sort{$ListItems->{$a}->{PlaceInOrder} <=> $ListItems->{$b}->{PlaceInOrder}} keys %{$ListItems}) {
      ...
      if($ListItems->{$ListItemID}->{ImageID} || $ListItems->{$ListItemID}->{CCImageID} || $ListItems->{$ListItemID}->{AmazonID}){
         ...
         if($imageCount < $ListCentral::SETUP::CONSTANTS{'IMAGE_IN_EMAIL_LIMIT'}){
            if($ListItems->{$ListItemID}->{ImageID}){
               ...
               my $src = $ListCentral::SETUP::URL . $ListCentral::SETUP::USER_CONTENT_PATH . "/" . $path . "/" . $ListItems->{$ListItemID}->{ImageID} . "M." . $Image->{Extension};
            ...
            $truncatedMsg = $ListCentral::SETUP::MESSAGES{'TRUNCATED_EMAIL_MESSAGE'};
         }
   ...
   $ListCreatorUser->{"UserURL"} = $ListCentral::SETUP::URL . $self->{UserManager}->getUserURL($ListCreatorUser);
   ...
   $Data{"EmailFrom"} = qq~by <a href=3D"$ListCentral::SETUP::URL$User->{UserURL}" style=3D"color:#1d687a;font-weight:bold;">$User->{Username}</a>~;

   $Data{"BodyTXT"} = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/list.txt", \%Data);
   $Data{"BodyHTML"} = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/list.html", \%Data);

   my $text = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/email_template.txt", \%Data);
   my $html = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/email_template.html", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager getListForm");

   my $template = "$ListCentral::SETUP::DIR_PATH/listpieces/listform.html";
   if($self->{UserManager}->{ThisUser}->{ID} == $List->{UserID}){
      $template =~ s/$ListCentral::SETUP::DIR_PATH/$ListCentral::SETUP::DIR_PATH\/owner/;
   ...
   my $ExtrElement = $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_extra_element.html", \%DataExtra);
   my $ExtraHiddenFields = $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/form_element_extra_hidden_fields.html", \%DataExtra);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager getListFormEdit");
   ...
   my $template = "$ListCentral::SETUP::DIR_PATH/owner/listpieces/listformedit.html";
   my $content = $self->processPage($template, \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListComments with $ListID");
   ...
   foreach my $ID (sort{$ListComments->{$b}->{CreateDate} <=> $ListComments->{$a}->{CreateDate}}keys %{$ListComments}) {
      ...
      $Comment->{'CreateDateFormatted'} = ListCentral::Utilities::Date::getShortHumanFriendlyDate($Comment->{'CreateDate'});
      ...
      if($List->{UserID} == $self->{UserManager}->{ThisUser}->{ID} || $Comment->{CommenterID} == $self->{UserManager}->{ThisUser}->{ID}){
         $Comments .= $self->processPage("$ListCentral::SETUP::DIR_PATH/owner/listpieces/list_comment.html", \%Comment);      
      }else{
         $Comments .= $self->processPage("$ListCentral::SETUP::DIR_PATH/listpieces/list_comment.html", \%Comment);      
      }      
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getListsByTag"); 
   ...
   my $pageCount = int($ListsByTagCount / $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'});
   if($ListsByTagCount % $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'}){
      $pageCount++;
   }
   ...
   my $SearchModule = new ListCentral::Utilities::Search('Debugger'=>$self->{Debugger}, 'DBManager'=>$self->{DBManager}, 
                                                   'UserManager'=>$self->{UserManager}, 'cgi' => $self->{cgi});
   my $ResultsRows = $SearchModule->processSearchResults("$ListCentral::SETUP::DIR_PATH/widgets/listing_row.html", $ListOfListByTag);
   ...
   $self->{Tag} = $Tag->{Name};
   my $Content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/tag_listing.html");
...
#############################################
   ...
   my $Ranker = new ListCentral::Utilities::Ranker(Debugger => $self->{Debugger}, DBManager => $self->{DBManager});
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getTagCloud"); 
   ...
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/3d_tagcloud.html"); 
...
#############################################
   ...
   use ListCentral::Utilities::TagCloud;
   my $TagCloud = new ListCentral::Utilities::TagCloud("Debugger" => $self->{Debugger}, "DBManager" => $self->{DBManager});
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getCSSLinks - UserID: $self->{cgi}->{UserID}");
   ...
   if($Browser ne "mozilla"){
      ...
      if($ListCentral::SETUP::USE_FIREBUG_LITE){
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getCSSLinksMultibox ");
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getUsersBoard");
   ...
   foreach my $ID(sort{$BoardMessages->{$b}->{CreateDate} <=> $BoardMessages->{$a}->{CreateDate}} keys %{$BoardMessages}) {
      $BoardMessages->{$ID}->{'CreateDateFormatted'} = ListCentral::Utilities::Date::getShortHumanFriendlyDate($BoardMessages->{$ID}->{CreateDate});
      ...
      $Board .= $self->processPage("$ListCentral::SETUP::DIR_PATH/board_rows.html", \%BoardRow);      
   }
   ...
   my $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/board.html");
   return $content;
}
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getMessages");

   if(! $self->{UserManager}->{ThisUser}->{ID}){
      $self->{ErrorMessages} =  $ListCentral::SETUP::MESSAGES{'MISC_ERROR'};
      $self->processPage("$ListCentral::SETUP::DIR_PATH/error.html");
   }

   my $MessagesObj = $self->{DBManager}->getTableObj("Messages");
   my $Messages = $MessagesObj->get_with_restraints("UserID = $self->{UserManager}->{ThisUser}->{ID}", 
                                          "CreateDate DESC", $ListCentral::SETUP::CONSTANTS{'MESSAGE_LIMIT'});
   ...
   my $today = ListCentral::Utilities::Date::getFullHumanFriendlyDate(time());
   my $todayKey; 
   ...
   foreach my $ID(sort{$Messages->{$b}->{CreateDate} <=> $Messages->{$a}->{CreateDate}} keys %{$Messages}) {
      ...
      $Data{"Date"} = ListCentral::Utilities::Date::getFullHumanFriendlyDate($Messages->{$ID}->{CreateDate});
      ...
      $DateHash{$dateKey}{HTML} .= $self->processPage("$ListCentral::SETUP::DIR_PATH/owner/message_row.html", \%Data);
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getUsersSpace");

   my $content;

   if($self->{cgi}->{UserID} == $ListCentral::SETUP::ABOUT_USER_ACCOUNT){
      $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/about/about.html");
   }else{
      $content = $self->getPage("$ListCentral::SETUP::DIR_PATH/users_space.html");
   }
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getSelectTag with $table and ListGroupID: $List->{ListGroupID}");
   ...
   if($table eq "Public"){
      ...
      my $options = "";
      foreach my $key (sort keys %ListCentral::SETUP::AMAZON_MODES) {
         if($key eq $self->{cgi}->{mode}){
            $options .= qq~<option value="$key" selected="selected">$ListCentral::SETUP::AMAZON_MODES{$key}</option>
                     ~;
         }else{
            $options .= qq~<option value="$key">$ListCentral::SETUP::AMAZON_MODES{$key}</option>
                     ~;
         }         
      }
      ...
      my $birthfield = $1;
      $select = ListCentral::Utilities::Date::getDateDropDowns($birthfield, "UserSettings.Birth$birthfield", "Birthday", $self->{cgi}->{"UserSettings.Birth$birthfield"});
   }else{
      my $TableObj = $self->{DBManager}->getTableObj($table);
      my $rows;
      my $selectBlank = "";
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getThisUserSelectTag with $table");
...
#####################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::isListRatingClickable");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} == $ListOwnerID && 
               $ListCentral::SETUP::CONSTANTS{'USERS_CANT_RATE_THEIR_OWN_LISTS'}){
      $Clickable = 0;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getBigListOfLists"); 
   ...
   my $pageCount = int($TopListCount / $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'});
   if($TopListCount % $ListCentral::SETUP::CONSTANTS{'LISTING_ROWS_LIMIT'}){
      $pageCount++;
   }

   if(!$pageCount){
      # Once this is working, this should never happen
      $self->{ErrorMessages} = $ListCentral::SETUP::MESSAGES{'NO_TOP_LISTS'};
      return $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/top_lists.html"); 
   }
   ...
   $self->{TopListsRows} = $SearchModule->processSearchResults("$ListCentral::SETUP::DIR_PATH/widgets/listing_row.html", $BigListOfList);
   if($TopListCount){
      my $TimeFrame = "";
   ...
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/top_lists.html"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getTopListsNavigation"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getTopUsers"); 
   ...
   foreach my $ID (sort{$TopUsers->{$b}->{LastLogin} <=> $TopUsers->{$a}->{LastLogin}} keys %{$TopUsers}) {
      ...
      $rows .= $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/top_users_rows.html", \%Data); 
   }

   $self->{TopUserRows} = $rows;
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/top_users.html"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getRecentComments"); 
   ...
   foreach my $ID (sort{$RecentComments->{$b}->{CreateDate} <=> $RecentComments->{$a}->{CreateDate}} keys %{$RecentComments}) {
      ...
      $RecentComments->{$ID}->{"CommenterURL"} = $ListCentral::SETUP::URL . $self->{UserManager}->getUserURL($Commenter);
      $self->{UserManager}->getUserAvatar($Commenter);
      $RecentComments->{$ID}->{"AvatarDisplay"} = $Commenter->{"AvatarDisplay"};
      #$RecentComments->{$ID}->{"CreateDateFormatted"} = ListCentral::Utilities::Date::getShortHumanFriendlyDate($RecentComments->{$ID}->{"CreateDate"});
      
      my %Data = ("Comments" => $RecentComments->{$ID});
      $rows .= $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_comments_rows.html", \%Data); 
   }

   $self->{RecentCommentsRows} = $rows;
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_comments.html"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getRecentLists"); 
   ...
   foreach my $ID (sort{$RecentLists->{$a}->{SortOrder} <=> $RecentLists->{$b}->{SortOrder}} keys %{$RecentLists}) {
      ...
      $rows .= $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_lists_rows.html", \%Data); 
      $count++;
      if($count == 6){
   ...
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_lists.html"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::getRecentSearches"); 
   ...
   foreach my $ID (sort{$RecentSearches->{$a}->{SortOrder} <=> $RecentSearches->{$b}->{SortOrder}} keys %{$RecentSearches}) {
      ...
      $RecentSearches->{$ID}->{TimeAgo} = ListCentral::Utilities::Date::getTimeSince($RecentSearches->{$ID}->{Date});
      ...
      $rows .= $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_searches_rows.html", \%Data); 
   }
   
   $self->{RecentSearchRows} = $rows;
   my $content = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/recent_searches.html"); 
...
#############################################
   ...
   if($self->{UserManager}->{ThisUser}->{ID} != $List->{UserID}){
      ...
      }elsif($ENV{HTTP_REFERER} !~ m/$ListCentral::SETUP::URL/ && $ENV{HTTP_REFERER} !~ m/$ListCentral::SETUP::URL2/){
         $Referrer = $ENV{HTTP_REFERER};
      }elsif($ENV{HTTP_REFERER} =~ m/$ListCentral::SETUP::URL/ && $ENV{HTTP_REFERER} =~ m/$ListCentral::SETUP::URL2/){
         $UserID = 0;
      }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager->getPageTitle $page");
   ...
   }elsif($self->{cgi}->{"ListID"} == $ListCentral::SETUP::CONSTANTS{'FAQ_LISTID'}){
      $pageTitle = "list central faqs";
   }elsif($page =~ m/lists/) {
      if($self->{PageOwner}->{ID} == $ListCentral::SETUP::ABOUT_USER_ACCOUNT){
         $pageTitle = "about list central";
      }else{
         $pageTitle = "lc: lists by $self->{PageOwner}->{Username}";
      }      
   }else{
      $pageTitle = $ListCentral::SETUP::CONSTANTS{'DEFAULT_PAGE_TITLE'};
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager->getThemeVisuals");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager::setThemeID self->{cgi}->{UserID}: $self->{cgi}->{UserID}, $self->{file_requested}");
...
Sends email to IT as per the email in ListCentral::SETUP::TO_IT_EMAIL
...
#############################################
   ...
   $self->{Debugger}->debug("ListCentral::Utilities::Mailer::sendWelcomeEmail");

   my %Data;
   $Data{"User"} = $User;

   my $subject = "Welcome to List Central";
   my $EmailBodyHTML = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/welcome.html", \%Data);
   my $EmailBodyTXT = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/welcome.txt", \%Data);

   $Data{"BodyHTML"} = $EmailBodyHTML;
   $Data{"BodyTXT"} = $EmailBodyTXT;

   my $EmailHTML = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/email_template.html", \%Data);
   my $EmailTXT = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/email_template.txt", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("ListCentral::Utilities::Mailer::sendNotificationEmail");
   ...
   $Data{"DoerURL"} = $ListCentral::SETUP::URL.$self->{UserManager}->getUserURL($Doer); 
   $Data{"DoerName"} = $Doer->{Username};
   $Data{"Date"} = ListCentral::Utilities::Date::getFullHumanFriendlyDate($Messages->{CreateDate});
   ...
   if($Messages->{Action} == 1){
      ...
      my $ListGroupName = $self->{ListGroupObj}->get_field_by_ID("Name", $List->{ListGroupID});
      $Data{"SubjectURL"} = $ListCentral::SETUP::URL.$self->{ListObj}->getListURL($List, $ListGroupName);
      $Data{"SubjectName"} = $List->{Name};
      $subject = "Your list has received a comment at List Central";
   }elsif($Messages->{Action} == 2){
      # Board Post
      $Data{"SubjectURL"} = $ListCentral::SETUP::URL.'/'.$self->{UserManager}->getUserURL($self->{UserManager}->{ThisUser});
      $Data{"SubjectName"} = "your board";
   ...
   my $EmailBodyHTML = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/notification.html", \%Data);
   my $EmailBodyTXT = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/notification.txt", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("ListCentral::ListManager::sendAccountDetailsEmail with user: $User->{Username}");

   my %Data;
   $Data{"User"} = $User;

   my $subject = "List Central account details";
   my $EmailBodyHTML = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/account_details.html", \%Data);
   my $EmailBodyTXT = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/account_details.txt", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("ListCentral::ListManager::sendFeedbackNotify ");
   ...
   my $EmailBodyHTML = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/feedback_notify.html", \%Data);
   my $EmailBodyTXT = $self->processPage("$ListCentral::SETUP::DIR_PATH/emails/feedback_notify.txt", \%Data);
   ...
   $Mailer->sendEmail($ListCentral::SETUP::FEEDBACK_TO_LISTS, 
                    $ListCentral::SETUP::MAIL_FROM_LISTS, 
                    "LC - New feedback", 
                    $EmailHTML, $EmailTXT, $boundary);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::getAvatar with param: $param");
   ...
   my $avatar = $self->processPage("$ListCentral::SETUP::DIR_PATH/widgets/avatar.html", \%Data);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager::getUnderAboutImage");
