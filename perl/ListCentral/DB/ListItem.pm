package ListCentral::DB::ListItem;
...
##########################################################
# ListCentral::DB::ListItem 
##########################################################

=head1 NAME

   ListCentral::DB::ListItem.pm

=head1 SYNOPSIS

   $ListItem = new ListCentral::DB::ListItem($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::getTopListItems with ListID $List->{ID} and query: $Query");
   
   my $limit = $ListCentral::SETUP::CONSTANTS{'NUMBER_OF_TOP_LIST_ITEMS_BRIEF_LIST_VIEW'};
   ...
   if($Query){
      ...
      my $sql = "SELECT * 
                FROM $ListCentral::SETUP::DB_NAME.ListItem 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::List->ListchangeListGroupToUncategorized with $StatusSet, $UserID");
   ...
   foreach my $ListItemStatusID(keys %{$ListItemStatus}) {
      my $sql = "UPDATE $ListCentral::SETUP::DB_NAME.ListItem INNER JOIN $ListCentral::SETUP::DB_NAME.List on ListItem.ListID = List.ID
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::getListItemsSample -> $ListHash->{Name}");
   ...
   foreach my $ItemID(sort{$Items->{$a}->{SortOrder} <=> $Items->{$b}->{SortOrder}} keys %{$Items}) {
      if($CurrentSpace >= $ListCentral::SETUP::CONSTANTS{'LIST_ITEM_SAMPLE_SPACES'}){
         $self->{Debugger}->debug("--- leaving forech with CurrentSpace: $CurrentSpace, ListCentral::SETUP::LIST_ITEM_SAMPLE_SPACES: $ListCentral::SETUP::LIST_ITEM_SAMPLE_SPACES");
         last;
      }
      ...
      if($Items->{$ItemID}->{AmazonID}){
         ...
         $imgsrc = $ListCentral::SETUP::USER_CONTENT_PATH . "/" . $ListHash->{UserDir} . "/" . 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListManager->getListItemsInfo");

   my $ImageObj = $self->{DBManager}->getTableObj("Image");
   my $LinkObj = $self->{DBManager}->getTableObj("Link");

   use ListCentral::Utilities::Amazon;
   my $Amazon = new ListCentral::Utilities::Amazon(DBManager => $self->{DBManager}, Debugger => $self->{Debugger},
                                                UserManager => $self->{UserManager});
...
#############################################
   ...
   $listName =~ s/'/&#39;/g;
   if(length($listName) > ($ListCentral::SETUP::CONSTANTS{'SAMPLE_LIST_NAME_WITH_IMAGES_LIMIT'} + 1)){
      $listName = substr($listName, 0, $ListCentral::SETUP::CONSTANTS{'SAMPLE_LIST_NAME_WITH_IMAGES_LIMIT'}) . "...";
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::getListItems with $List->{ID}");
   ...
   my $limit; my $start;
   if($extraCount > $ListCentral::SETUP::CONSTANTS{'IMAGE_LIST_ITEM_LIMIT'}){
      $start = ($page - 1) * $ListCentral::SETUP::CONSTANTS{'IMAGE_LIST_ITEM_LIMIT'}; 
      $limit = $ListCentral::SETUP::CONSTANTS{'IMAGE_LIST_ITEM_LIMIT'};
   }else{
      $start = ($page - 1) * $ListCentral::SETUP::CONSTANTS{'LIST_ITEM_LIMIT_PER_PAGE'}; 
      $limit = $ListCentral::SETUP::CONSTANTS{'LIST_ITEM_LIMIT_PER_PAGE'};
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::GetMinPlaceInOrder with ListID: $ListID, and ListItemStatusID: $ListItemStatusID");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::getMaxPlaceInOrder with ListID: $ListID, and ListItemStatusID: $ListItemStatusID");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::ListItem::clearListItemExtras with $ListItem->{ID}, $OneToKeep");
