...
=head1 NAME
   ...
   ListCentral::DB::Tag.pm

=head1 SYNOPSIS

   $Tag = new ListCentral::DB::Tag($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag::getListsByTag with TagID : $TagID");
   ...
   my $sql = "SELECT List.*, ListTag.*
              FROM $ListCentral::SETUP::DB_NAME.ListTag,  $ListCentral::SETUP::DB_NAME.List, $ListCentral::SETUP::DB_NAME.User 
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag->getTagURL with $Tag->{ID}");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag::getPublicTags");

   my %Tag;
   my $sql = "SELECT distinct(Tag.Name), Tag.*
              FROM $ListCentral::SETUP::DB_NAME.Tag, $ListCentral::SETUP::DB_NAME.ListTag, $ListCentral::SETUP::DB_NAME.List, $ListCentral::SETUP::DB_NAME.User
              WHERE List.Public = 1 
              ...
              LIMIT $ListCentral::SETUP::CONSTANTS{'THREE_D_TAG_CLOUD_LIMIT'}";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
         $Tag{$hash->{ID}} = $hash;
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag->getListTags with $ListID");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::ListsManager->getListTagCloud with $ListID");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Tag::reduceTagCount with $TagID");

   my $sql = "UPDATE $ListCentral::SETUP::DB_NAME.Tag SET TagCount = TagCount - 1 WHERE ID = $TagID";
   my $stm = $self->{dbh}->prepare($sql);
   if ($self->{dbh}->do($sql)) {
      # Good Stuff
   }else {
