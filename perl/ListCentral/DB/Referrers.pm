package ListCentral::DB::Referrers;
...
##########################################################
# ListCentral::DB::Referrers 
##########################################################

=head1 NAME

   ListCentral::DB::Referrers.pm

=head1 SYNOPSIS

   $Referrers = new ListCentral::DB::Referrers($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::Referrers saveReferrer");
...
#############################################
   ...
   my $Referrer;
   my $sql = "SELECT * FROM $ListCentral::SETUP::DB_NAME.Referrers WHERE Name = \"$referrer\"";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
         $Referrer = $hash;
