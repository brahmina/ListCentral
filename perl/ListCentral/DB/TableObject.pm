package ListCentral::DB::TableObject;
...
##########################################################
# ListCentral::DB::TableObject 
##########################################################

=head1 NAME

   ListCentral::DB::TableObject.pm

=head1 SYNOPSIS

   $TableObject = new ListCentral::DB::TableObject($dbh, $debug);
...
=head2 ListCentral::DB::TableObject Constructor
...
########################################################
   ...
   my $package_name = $ListCentral::SETUP::PACKAGE_NAME . "::DB::";
...
=item B<Parameters :>
   ...
   1. $self - Reference to a ListCentral::DB::TableObject object
...
#############################################
   ...
   $self->{Debugger}->debugNow("in ListCentral::DB::$self->{Table}::store with");
   ...
   if($self->{HTMLFields}){
      foreach my $htmlField( @{$self->{HTMLFields}}){
         ...
         my $badHTML = ListCentral::Utilities::StringFormator::hasBadHTML($TableObj_hash_ref->{$htmlField});
         if($badHTML == 1){
            return $ListCentral::SETUP::MESSAGES{'UNPERMITTED_HTML'};
         }elsif($badHTML ne ""){
   ...
   #if($self->{SpamFields}){
   #   use ListCentral::Utilities::SpamCatcher;
   #   my $SpamCatcher = new ListCentral::Utilities::SpamCatcher(Debugger => $self->{Debugger});
   ...
   my $fields = ""; 
   my $values = "";
   my $sql_insert = "INSERT INTO $ListCentral::SETUP::DB_NAME.$self->{Table} (";
   ...
   if ($stm->execute()){
      ...
      return $ListCentral::SETUP::MESSAGES{'INSERT_ERROR'};; 
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::$self->{Table}::get_by_ID with $ID");
   ...
   my $sql_select = "SELECT *$dates
                     FROM $ListCentral::SETUP::DB_NAME.$self->{Table} 
                     WHERE ID = $ID
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::get_by_field with $field, $value");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::$self->{Table}::get_field_by_ID with $field, $ID");
   ...
   my $result;
   my $sql = "SELECT $field FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE ID = $ID AND $self->{StatusField} > 0";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::$self->{Table}::get_with_restraints with $where_clause");
   ...
   my $sql = "SELECT * FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE $where_clause AND $self->{StatusField} > 0 $orderBy $limit";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::get_all");
   ...
   my %TableObject;
   my $sql = "SELECT * FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE $self->{StatusField} > 0";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
         $TableObject{$hash->{ID}} = $hash;
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::get_count");

   my $count;
   my $sql = "SELECT COUNT(ID) AS Count FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE $self->{StatusField} > 0";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
         $count = $hash->{Count};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::get_count_with_restraints");

   my $count;
   my $sql = "SELECT COUNT(ID) AS Count FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE $where_clause AND $self->{StatusField} > 0";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::get_distinct_field_count");

   my $count;
   my $sql = "SELECT COUNT(DISTINCT($field)) AS Count FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE $self->{StatusField} > 0";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
         $count = $hash->{Count};
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::$self->{Table}::get_distinct_field_count_from_with_restraints");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::TableObject::delete with $ID");

   my $sql = "DELETE FROM $ListCentral::SETUP::DB_NAME.$self->{Table} WHERE ID = $ID";
   my $stm = $self->{dbh}->prepare($sql);
   if ($self->{dbh}->do($sql)) {
      # Good Stuff
   }else {
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::$self->{Table}::update with $field, $value, $ID");
   ...
   if($self->{HTMLFields}){
      foreach my $htmlField( @{$self->{HTMPFields}}){
         ...
         if($htmlField eq $field){
            my $badHTML = ListCentral::Utilities::StringFormator::hasBadHTML($value);
            if($badHTML == 1){
               return $ListCentral::SETUP::MESSAGES{'UNPERMITTED_HTML'};
            }elsif($badHTML ne ""){
               $value .= $badHTML;
   ...
   my $sql = "UPDATE $ListCentral::SETUP::DB_NAME.$self->{Table} SET $field = \"$value\" WHERE ID = $ID";
   my $stm = $self->{dbh}->prepare($sql);
   if ($self->{dbh}->do($sql)) {
      # Good Stuff
   }else {
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::Utilities::Search::runSQLSelect with $sql");
