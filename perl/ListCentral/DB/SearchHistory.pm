package ListCentral::DB::SearchHistory;
...
##########################################################
# ListCentral::DB::SearchHistory 
##########################################################

=head1 NAME

   ListCentral::DB::SearchHistory.pm

=head1 SYNOPSIS

   $SearchHistory = new ListCentral::DB::SearchHistory($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::SearchHistory::getRecentSearches");

   if(! $limit){
      $limit = $ListCentral::SETUP::CONSTANTS{'RECENT_SEARCHES_LIMIT'};
   }
   ...
   my $sql = "SELECT Count(Query) AS Count, Query, MAX(from_unixtime(Date)) AS FormattedDate, Date, 
                     ID, ResultsCount
              FROM $ListCentral::SETUP::DB_NAME.SearchHistory
              WHERE Status > 0
              GROUP BY Date
              ORDER BY Date DESC
              LIMIT $limit";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::SearchHistory:: getTopSearches");
