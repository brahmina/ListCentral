package ListCentral::DB::CCImage;
...
##########################################################
# ListCentral::DB::CCImage 
##########################################################

=head1 NAME

   ListCentral::DB::CCImage.pm

=head1 SYNOPSIS

   $CCImage = new ListCentral::DB::CCImage($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::CCImage::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::CCImage::getByFlkrID with $FlkrID");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::CCImage saveCCImage");
   ...
   if($CCImageCurrent->{ID}){
      ...
      my $CCImage = new ListCentral::Utilities::CCImage(Debugger => $self->{Debugger}, DBManager => $self->{DBManager});
