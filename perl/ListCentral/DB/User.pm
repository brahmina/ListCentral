...
=head1 NAME
   ...
   ListCentral::DB::User.pm

=head1 SYNOPSIS

   $User = new ListCentral::DB::User($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getActiveUsers");
   ...
   my $sql = "SELECT * 
              ...
              LIMIT $ListCentral::SETUP::ACTIVE_USER_REPORT_ROWS";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getTopUsers");

   my %ActiveUsers;
   my $sql = "SELECT * 
              FROM User 
              WHERE Status > 0 AND ID != $ListCentral::SETUP::ABOUT_USER_ACCOUNT
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getPopularUsers");

   my %PopularUsers;
   my $sql = "SELECT * FROM User WHERE Status > 0 ORDER BY PopularityScore LIMIT $ListCentral::SETUP::POPULAR_USER_REPORT_ROWS";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getTroublesomeUers");

   my %TroublesomeUsers;

   my $sql = "SELECT COUNT(DISTINCT UserID) AS Count, User.*
              FROM ComplaintsAgainstUser INNER JOIN $ListCentral::SETUP::DB_NAME.User ON User.ID = ComplaintsAgainstUser.UserID
              WHERE User.Status > 0 AND ComplaintsAgainstUser.Status > 0
              GROUP BY UserID
              ORDER BY Count
              LIMIT $ListCentral::SETUP::TROUBLESOME_USER_REPORT_ROWS";
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getUserByEmail $Email");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getUserByUsername with $Username");
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getUserModifiers");

   my %UserModifiers;
   my $sql_select = "SELECT User.ID, UserSettings.Modifier
                     FROM $ListCentral::SETUP::DB_NAME.User, $ListCentral::SETUP::DB_NAME.UserSettings
...
#############################################
   ...
   $self->{Debugger}->debug("in ListCentral::DB::User::getSubscribedUsers");
