...
=head1 SYNOPSIS
   ...
   $Editor = new Lists::Editor($dbh, $debug);

=head1 DESCRIPTION

Used to manage, and maintain the List application

=head2 Lists::Editor Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Editor constructor");
...
=item B<Parameters :>
   ...
   1. $self - Reference to a Lists::ListManager object
...
#####################################
   ...
   $self->{Debugger}->debug("in Lists::Editor::doEditInPlace");
   ...
   if($self->{UserManager}->{ThisUser}->{ID} != $UserID){
      ...
      if($self->{cgi}->{"Table"}){
         if($self->{cgi}->{"Value"}){
            my $value = Lists::Utilities::StringFormator::unquoteText($self->{cgi}->{"Value"});
            if($self->{cgi}->{'Field'} ne "Description"){
               $value = Lists::Utilities::StringFormator::htmlEncode($value);
            }elsif($self->{cgi}->{'Field'} eq "Description" && Lists::Utilities::StringFormator::hasBadHTML($self->{cgi}->{"Value"})){
               $value = Lists::Utilities::StringFormator::htmlEncode($value);
            }
...
#####################################
   ...
   $self->{Debugger}->debug("in Lists::Editor::getItemsUserID");
...
#####################################
   ...
   $self->{Debugger}->debug("in Lists::Editor::getEditableListGroups");
...
#####################################
   ...
   $self->{Debugger}->debug("in Lists::Editor::getEditableStatusSet");
...
#############################################
   ...
   #$self->{Debugger}->debug("in Lists::ListManager::getEditableListItem");
...
#############################################
   ...
   # When is this outter tag required?
   $self->{Debugger}->debug("in Lists::Editor::getEditableListItem $ListItemID & $Item");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Editor->updateListItem with $value");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Editor->updateListGroups with $Name");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Editor->updateStatusSet $StatusSetID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Editor->updateList");
