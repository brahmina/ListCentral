...
=head2 Lists::UserManager Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager constructor");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::ListsManager::UserManager->addUser");
   ...
   if(!$Mailer->emailValid($Email)){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'INVALID_EMAIL'} . "<br />";
   }
   if($self->{cgi}->{"User.Email"} eq ""){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'BLANK_EMAIL'} . "<br />"; 
   }
   if($self->{cgi}->{"User.Username"} eq ""){   
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'BLANK_USERNAME'} . "<br />"; 
   }
   if($self->{cgi}->{"User.Password"} eq ""){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'BLANK_PASSWORD'} . "<br />"; 
   }
   if($self->{cgi}->{"User.Name"} eq ""){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'BLANK_NAME'} . "<br />"; 
   }
   if($self->{cgi}->{"User.Password"} ne $self->{cgi}->{"User.PasswordConfirm"}){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'PASSWORD_NONMATCH'} . "<br />"; 
   }
   if($self->{cgi}->{"UserSettings.BirthDay"} eq ""){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'BLANK_BIRTHDAY'} . "<br />"; 
   ...
   if(! Lists::Utilities::Date::olderThan12($self->{cgi}->{"UserSettings.BirthDay"}, $self->{cgi}->{"UserSettings.BirthMonth"}, $self->{cgi}->{"UserSettings.BirthYear"})){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'TOO_YOUNG'} . "<br />"; 
   }
   ...
   if(!$valid){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'FAIL_RECAPTCHA'} . "<br /><br />"; 
   }
   ...
   if($ErrorMessage eq ""){
      foreach (keys %{$Users}) {
         if($Users->{$_}){
            $ErrorMessage .= $Lists::SETUP::MESSAGES{'DUPLICATE_EMAIL'} . "<br />";
         }  
      }
   }

   if($Lists::SETUP::BETA_INVITE_ONLY && ! $self->checkBetaInvite($Email)){
      return "BETA";
   }
   ...
   if($ErrorMessage eq ""){
      if($User->{ID}){
         $ErrorMessage .= $Lists::SETUP::MESSAGES{'DUPLICATE_USERNAME'} . "<br />"; 
      }  
   }

   if($ErrorMessage eq ""){
      my $Password = $self->{cgi}->{"User.Password"};
      if(length($Password) < $Lists::SETUP::PASSWORD_MIN_LENGTH || ! $Password =~ m/^(?=.*(\d|[^a-zA-Z]))(?!.*\s).{6,15}$/){
         $ErrorMessage .= $Lists::SETUP::MESSAGES{'INVALID_PASSWORD'} . "<br />"; 
      }
   }

   # Only alphanumeric
   if($self->{cgi}->{"User.Username"} =~ m/\@/){
      $ErrorMessage .= $Lists::SETUP::MESSAGES{'NO_AT_IN_USERNAME'} . "<br />"; 
   }
   #if($self->{cgi}->{"User.Name"} =~ m/[^a-zA-Z0-9,\s_-]/){
   #   $ErrorMessage .= $Lists::SETUP::MESSAGES{'ONLY_ALPHANUMERIC_NAME'} . "<br />"; 
   #}
   ...
   if($ErrorMessage eq ""){
      ...
      if(!$UserID){
         $ErrorMessage = $Lists::SETUP::MESSAGES{'MISC_ERROR'};
      }else{
         # All good, set cookie
...
=item B<Parameters :>
   ...
   1. $self - Reference to a Lists::UserManager object
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::doLogin");
   ...
   if($User->{ID}){
      if($User->{Status} == 1){
         if($User->{Password} eq $UserPassword){
            ...
            # Incorrect password
            return 0, $Lists::SETUP::MESSAGES{'INCORRECT_PASSWORD'};
         }
      }else{
         # Deatctivated Account
         return -1, $Lists::SETUP::MESSAGES{'DEACTIVATED_ACCOUNT'};
      }
   }else{
      # No sure user name
      return 0, $Lists::SETUP::MESSAGES{'INCORRECT_USERNAME'};
   }
}
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::setDefaultSettings,  $UserID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::createUsersDirectory with UserID: $UserID ");
   ...
   while($UserIDTemp =~ m/^(\d)/){
      my $digit = $1;
      $directory .= "$digit/";
      my $dir = "$Lists::SETUP::USER_CONTENT_DIRECTORY/$directory";
      if(! -e $dir){
         my $output = `mkdir $dir`;
         if($output){
            $self->{Debugger}->log("Error: Lists::UserManager->createUsersDirectory, couldn't make directory $dir - $output");
         }
      }
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUsersDirectory with UserID: $UserID"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::changeUsersDirectory with UserID: $UserID, $oldUsername, $newUsername"); 

   my $newUserDir = $Lists::SETUP::USER_CONTENT_DIRECTORY . "/" . $self->getUsersDirectory($UserID);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUsersUsername with UserID: $UserID"); 
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::checkLogin ");
   ...
   if($cookie_in =~ m/ListCentral=(.+)/){
      ...
      $UserID = $crypt->decrypt($decoded, $Lists::SETUP::ENCRYPT_KEY);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::doLogout ");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::setNormalCookie"); 

   # Encrypt the userID before setting it
   my $crypt = Crypt::Lite->new( debug => 0 );
   my $encrypted = $self->encrypt($UserID, $Lists::SETUP::ENCRYPT_KEY);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::setPersistentCookie with UserID: $UserID"); 

   my $RandomString = Lists::Utilities::StringFormator::randomString($Lists::SETUP::LENGTH_OF_PERSISTENT_COOKIE_RANDOM_STRING);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::checkPersistentCookie ");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getThisUserInfo ");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUserInfo User: $User->{ID}, UserSettings: $self->{UserSettings}, ClearCache: $self->{ClearCache}");
   ...
   if($self->{UserCache}->{$User->{ID}} && !$self->{ClearCache}){
      ...
      if(! $UserSettings->{PrivacyAge} && !$self->{UserSettings}){
         ...
         $User->{"Age"} = Lists::Utilities::Date::getAge($User->{"BirthDay"}, $User->{"BirthMont"}, $User->{"BirthYear"});
      }
      ...
      $User->{"MemberSince"} = Lists::Utilities::Date::getHumanFriendlyDate($User->{"CreateDate"});
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUserAvatar ");
   ...
   if($User->{"GravatarEmail"}){
      my $Gravatar = new Lists::Utilities::Gravatar(Debugger=>$self->{Debugger}, DBManager=>$self->{DBManager});
      $User->{"Gravatar"} = $Gravatar->getGravatarURL($User->{ID}, $User->{"GravatarEmail"});
   }else{
      $User->{"Gravatar"} = $Lists::SETUP::AVATAR_DEFAULT_IMAGE;
   }
   ...
   if($User->{"Avatar"}){
      ...
      my $path = $Lists::SETUP::USER_CONTENT_PATH . '/' . $User->{"UserDir"};
      $User->{"AvatarLink"} = $path . "/" . $User->{"Avatar"};
   }else{
      $User->{"AvatarLink"} = $Lists::SETUP::AVATAR_DEFAULT_IMAGE;
   }
   ...
   if($User->{"GravatarOrAvatar"} eq "G"){
      ...
      if($User->{"GravatarEmail"}){
         ...
         $User->{"AvatarDisplay"} = $Lists::SETUP::AVATAR_DEFAULT_IMAGE;
      }
      $User->{"GravatarSelected"} = "checked";
      $User->{"AvatarSelected"} = "";
   }else{
      if($User->{"Avatar"}){
         $User->{"AvatarDisplay"} = $User->{"AvatarLink"};#$Lists::SETUP::USER_CONTENT_PATH . "/" . $User->{"Avatar"};
      }else{
         $User->{"AvatarDisplay"} = $Lists::SETUP::AVATAR_DEFAULT_IMAGE;
      }
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUserURL with ID $User->{ID} ");
...
#############################################
   ...
   my $crypt = Crypt::Lite->new( debug => 0 );
   my $encrypted = $crypt->encrypt($string, $Lists::SETUP::ENCRYPT_KEY);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager->decrypt with $string");

   my $crypt = Crypt::Lite->new( debug => 0 );
   my $decrypted = $crypt->decrypt($string, $Lists::SETUP::ENCRYPT_KEY);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager:checkBetaInvite with $Email");
...
#############################################
   ...
   $ctx->add($password);
   $ctx->add($Lists::SETUP::SALT);
   my $hashedPasswd = encode_base64($ctx->digest . $Lists::SETUP::SALT ,'');
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::generatePassword");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUsersEncryptedPassword $UserID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::userListCount $UserID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::UserManager::getUsersAmazonCountry");
