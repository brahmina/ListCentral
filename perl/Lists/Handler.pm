...
###################################
   ...
   my $Debugger = new Lists::Debugger(debug=>$debug, request=>$r, cgi => $cgi);
   ...
   if($uri =~ m'/tagged/(\w+)/(\d+)/(.+)\.html'){
      ...
      $cgi->{"TagID"} = $2;
      my $file = $3;
      $file_requested = $Lists::SETUP::DIR_PATH . "/" . $file . ".html";
      ...
      my $file = $3;
      $file_requested = $Lists::SETUP::DIR_PATH . "/" . $file . ".html";
      if($cgi->{UserID} == $Lists::SETUP::ABOUT_USER_ACCOUNT){
         $file_requested = $Lists::SETUP::DIR_PATH . "/" . "about.html";
      }
      $r->content_type("text/html"); 
   }elsif($uri =~ m'/about/(\d+)/\w+/help_page.html'){
      # List
      $cgi->{HelpPageID} = $1;
      $file_requested = $Lists::SETUP::DIR_PATH . "/about/help_page.html";
      $r->content_type("text/html"); 
   }
   ...
   my $AdServer = new Lists::Utilities::AdServer(Debugger=>$Debugger, DBManager=>$DBManager, Request=>$r,);

   # Get any cookies and instantiate the user object
   my $ListsManager = new Lists::ListsManager(Debugger=>$Debugger, 
                                              cgi=>$cgi, Request=>$r, AdServer => $AdServer,
                                              UserManager=>$UserManager, DBManager=>$DBManager);
   my $Printer = new Lists::Printer(cgi=>$cgi, Debugger=>$Debugger, dbh=>$dbh, 
   ...
   if($Referrer !~ m/$Lists::SETUP::URL/ && $Referrer !~ m/$Lists::SETUP::URL2/){
