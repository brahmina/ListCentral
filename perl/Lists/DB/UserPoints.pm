...
=head1 NAME
   ...
   Lists::DB::UserPoints.pm

=head1 SYNOPSIS

   $UserPoints = new Lists::DB::UserPoints($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::UserPoints::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::UserPoints::getByUserID with $UserID");

   my $UserPoints;
   my $sql_select = "SELECT * 
                     FROM $Lists::SETUP::DB_NAME.UserPoints 
                     WHERE UserID = $UserID
                           AND Status > 0";
