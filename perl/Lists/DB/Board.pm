...
=head1 SYNOPSIS
   ...
   $Board = new Lists::DB::Board($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Board::getBoardMessages");

   my $BoardMessageCount;
   my $sql = "SELECT COUNT(Board.ID) AS Count 
              FROM $Lists::SETUP::DB_NAME.Board, $Lists::SETUP::DB_NAME.User
   ...
   my $pageCount = int($BoardMessageCount / $Lists::SETUP::CONSTANTS{'BOARD_POSTS_PER_PAGE'});
   if($BoardMessageCount % $Lists::SETUP::CONSTANTS{'BOARD_POSTS_PER_PAGE'}){
      $pageCount++;
   }

   my $start = ($page - 1) * $Lists::SETUP::CONSTANTS{'BOARD_POSTS_PER_PAGE'}; 
   my $limit = $start . ", " . $Lists::SETUP::CONSTANTS{'BOARD_POSTS_PER_PAGE'};
