...
=head1 SYNOPSIS
   ...
   $Hitlog = new Lists::DB::Hitlog($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Hitlog::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Hitlog::store_hitlog_entry");
   ...
   if($ENV{REMOTE_ADDR} eq $Lists::SETUP::CONSTANTS{'MONITORING'}){
      # Don't record the monitoring service
      return;
   }

   if($ip ne $MarilynBurgess::Lists::SETUP::MY_IP){
      my $time = time();
      my $sql_hitlog = "INSERT INTO $Lists::SETUP::DB_NAME.Hitlog (IP, Date, URI, Referer, UserID, UserAgent) 
                                   VALUES (?,?,?,?,?,?)";
