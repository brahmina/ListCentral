...
=head1 SYNOPSIS
   ...
   $List = new Lists::DB::List($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List->changeListGroupToUncategorized with $ListGroupID, $UserID");

   my $sql = "UPDATE $Lists::SETUP::DB_NAME.List SET ListGroupID = 1 WHERE UserID = $UserID AND ListGroupID = $ListGroupID";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getPopularLists $UserID");

   my $limit;
   my $UserClause = "";
   if($UserID){
      $UserClause = "AND UserID = $UserID";
      $limit = $Lists::SETUP::POPULAR_LIST_BREAKDOWN_ROWS;
   }else{
      $limit = $Lists::SETUP::POPULAR_LIST_REPORT_ROWS;
   }
   ...
   my $sql = "SELECT List.*, ListPoints.MostRecent, ListPoints.24Hours, ListPoints.72Hours, ListPoints.LastWeek,
		     ListPoints.LastMonth, ListPoints.LastYear, ListPoints.AllTime, User.Username
              FROM $Lists::SETUP::DB_NAME.List, $Lists::SETUP::DB_NAME.User, $Lists::SETUP::DB_NAME.ListPoints
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getListURL ID: $List->{ID}, Name: $List->{Name}, Group: $ListGroup");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getListFavCount $ListID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getPopularLists with page: $page");
   ...
   my $sql = "SELECT List.*, ListPoints.ListPoints, ListPoints.RankingHN, ListPoints.RankingReddit, 
              ...
              WHERE List.UserID = User.ID AND User.Status > 0 
                    ...
                    AND List.UserID != $Lists::SETUP::ABOUT_USER_ACCOUNT
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getNewLists with page: $page");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getActiveLists with page: $page");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getAllPublicLists");

   my %Lists;
   my $sql = "SELECT List.*
              FROM $Lists::SETUP::DB_NAME.List, $Lists::SETUP::DB_NAME.User
              WHERE List.UserID = User.ID AND User.Status > 0
                    AND List.UserID != $Lists::SETUP::ABOUT_USER_ACCOUNT
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getListsCount");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::List::getRecentLists");
   ...
   my $sql = "SELECT List.*
              ...
              LIMIT $Lists::SETUP::CONSTANTS{'RECENT_LISTS_LIMIT'}";
   my $sortOrder = 1;
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::ListManager::getListInfo $ListHash->{ID}");
   ...
   if(length($ListHash->{ListName}) > $Lists::SETUP::CONSTANTS{'MAX_LIST_NAME_LENGTH_LISTING'}){
      $ListHash->{ListName} = substr ($ListHash->{ListName}, 0, ($Lists::SETUP::CONSTANTS{'MAX_LIST_NAME_LENGTH_LISTING'} - 2)) . "...";
   }
   ...
   # Dates Formatted
   $ListHash->{FormattedCreateDate} = Lists::Utilities::Date::getShortHumanFriendlyDate($ListHash->{CreateDate});
   if($ListHash->{Public}){
      $ListHash->{DateMadePublicFormatted} = Lists::Utilities::Date::getShortHumanFriendlyDate($ListHash->{DateMadePublic});
   }else{
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::ListManager::getBriefListInfo $ListHash->{ID}");
