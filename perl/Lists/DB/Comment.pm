...
=head1 SYNOPSIS
   ...
   $Comment = new Lists::DB::Comment($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Comment::getListComments");

   my $ListID = $List ->{ID};
   my $CommentCount = $self->getListCommentCount($List->{ID});

   my $pageCount = int($CommentCount / $Lists::SETUP::CONSTANTS{'COMMENT_LIMIT_PER_PAGE'});
   if($CommentCount % $Lists::SETUP::CONSTANTS{'COMMENT_LIMIT_PER_PAGE'}){
      $pageCount++;
   }

   my $start = ($page - 1) * $Lists::SETUP::CONSTANTS{'COMMENT_LIMIT_PER_PAGE'}; 
   my $limit = $start . ", " . $Lists::SETUP::CONSTANTS{'COMMENT_LIMIT_PER_PAGE'};

   my %ListComments;
   my $sql = "SELECT Comment.* 
              FROM $Lists::SETUP::DB_NAME.Comment, $Lists::SETUP::DB_NAME.User
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Comment::getListCommentCount");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::User::getRecentComments");
   ...
   my $sql = "SELECT Comment.ID, Comment, Commenter, CommenterID, Comment.ListID, ListGroup.Name AS ListGroup, 
              ...
              FROM $Lists::SETUP::DB_NAME.Comment, $Lists::SETUP::DB_NAME.List, $Lists::SETUP::DB_NAME.User, $
                   Lists::SETUP::DB_NAME.ListGroup
              ...
              LIMIT $Lists::SETUP::CONSTANTS{RECENT_COMMENTS_WIDGET_LIMIT}";
   my $stm = $self->{dbh}->prepare($sql);
   if ($stm->execute()) {
      while (my $hash = $stm->fetchrow_hashref()) {
