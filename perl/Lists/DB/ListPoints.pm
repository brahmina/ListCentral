...
=head1 NAME
   ...
   Lists::DB::ListPoints.pm

=head1 SYNOPSIS

   $ListPoints = new Lists::DB::ListPoints($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::ListPoints::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::ListPoints::getByListID with $ListID");

   my $ListPoints;
   my $sql_select = "SELECT * 
                     FROM $Lists::SETUP::DB_NAME.ListPoints 
                     WHERE ListID = $ListID
                           AND Status > 0";
