...
=head1 SYNOPSIS
   ...
   $Help = new Lists::DB::Help($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::Help::init");
...
#############################################
   ...
   $self->{Debugger}->debugNow("in Lists::DB::Help::getListings");

   my %HelpListings;
   my $sql_select = "SELECT ID, Name, Url, Ordering
                     FROM $Lists::SETUP::DB_NAME.Help 
...
#############################################
   ...
   $self->{Debugger}->debugNow("in Lists::DB::Help::getNextHelpPosition");
