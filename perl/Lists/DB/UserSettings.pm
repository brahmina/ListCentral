...
##########################################################
# Lists::DB::UserSettings 
##########################################################

=head1 NAME

   Lists::DB::UserSettings.pm

=head1 SYNOPSIS

   $UserSettings = new Lists::DB::UserSettings($dbh, $debug);
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::UserSettings::init");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::UserSettings::getByUserID with $UserID");

   my $UserSettings;
   my $sql_select = "SELECT * 
                     FROM $Lists::SETUP::DB_NAME.UserSettings 
                     WHERE UserID = $UserID
                           AND Status > 0";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::DB::UserSettings::getFieldByUserID with $field, $UserID");

   my $result;
   my $sql = "SELECT $field FROM $Lists::SETUP::DB_NAME.UserSettings WHERE UserID = $UserID AND Status > 0";
