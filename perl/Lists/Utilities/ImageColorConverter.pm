package Lists::Utilities::ImageColorConverter;
use strict;
...
##########################################################
# Lists::Utilities::ImageColorConverter 
##########################################################

=head1 NAME

   Lists::Utilities::ImageColorConverter.pm

=head1 SYNOPSIS

   $ImageColorConverter = new Lists::Utilities::ImageColorConverter(Debugger);
...
=head2 Lists::Utilities::ImageColorConverter Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::ImageColorConverter constructor");
...
image source directory (Lists::SETUP::SOURCE_THEME_IMAGES_DIR) the image colors 
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::ImageColorConverter::convertImages with ThemeID = $ThemeID");

   # Make the new dir
   my $newthemedir = $Lists::SETUP::SOURCE_THEME_IMAGES_DIR;
   $newthemedir =~ s/Base//;
   ...
   opendir(BASE_DIR, $Lists::SETUP::SOURCE_THEME_IMAGES_DIR);
   ...
   foreach my $img(@files) {
      if($img =~ m/png$/i || $img =~ m/gif/i || $img =~ m/jpg/i || $img =~ m/jpeg/){
         ...
         my $imagefile = $Lists::SETUP::SOURCE_THEME_IMAGES_DIR ."/".$img;
         ...
         for my $y (0..($height-1)){
            for my $x (0..($width-1)){
               ...
               for(my $i = 0; $i < @Lists::SETUP::THEME_BASE_COLORS; $i++ ) {
                  #$self->{Debugger}->debug("$color eq $Lists::SETUP::THEME_BASE_COLORS[$i] ??");
                  if($color eq $Lists::SETUP::THEME_BASE_COLORS[$i] && $color ne $colorArray->[$i]){
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon::convertImages with ThemeID = $ThemeID");
      ...
      if($img =~ m/png$/i || $img =~ m/gif/i || $img =~ m/jpg/i || $img =~ m/jpeg/){
         ...
         for my $y (0..($height-1)){
            for my $x (0..($width-1)){
               ...
               for(my $i = 0; $i < @Lists::SETUP::THEME_BASE_COLORS; $i++ ) {
                  $self->{Debugger}->debug("$old_color eq $Lists::SETUP::THEME_BASE_COLORS[$i]  ??");
                  if($old_color eq $Lists::SETUP::THEME_BASE_COLORS[$i] && $old_color ne $colorArray->[$i]){
