package Lists::Utilities::CCImage;
use strict;
...
=head1 NAME
   ...
   Lists::Utilities::CCImage.pm

=head1 SYNOPSIS

   $CCImage = new Lists::Utilities::CCImage(Debugger);
...
=head2 Lists::Utilities::CCImage Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::CCImage constructor");

   #$self->{'FlickrKey'} = '206c3ec5c02a71e4ed8bc418774dc8e4';
   #$self->{'FlickrSecret'} = 'e0a2d8d4c17825ef';

   # Marilyn Burgess account for the List Central app
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::CCImage::getTopSearchResults with $query, $page");
   ...
   my $response = $api->execute_method('flickr.photos.search', {
                ...
                'per_page' => $Lists::SETUP::CONSTANTS{'CCIMAGE_CHOICE_LIMIT'},
                'page' => $page,
                'media' => 'photos',
                'tag_mode' => 'any',
                'tags' => $tags,
   ...
   foreach my $ID(keys %Photos) {
      if($count < $Lists::SETUP::CONSTANTS{'CCIMAGE_CHOICE_LIMIT'}){
         $self->{Debugger}->debug("Choice - $ID");

         my %Data = ("CCImageChoice" => $Photos{$ID});
         $CCImageChoices .= $self->getBasicPage("$Lists::SETUP::DIR_PATH/Utilities/CCImage/choices_rows.html", \%Data);
         $count++;
      }
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::CCImage::getImageByID with $ID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::CCImage->setCCImage with FlkrID: $FlkrID");
   ...
   my $url = Lists::Utilities::StringFormator::htmlDecode($CCImage->{'URL'});
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::CCImage->getBasicPage with page $page, $Data");
