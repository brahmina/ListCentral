package Lists::Utilities::Amazon;
use strict;

use Lists::SETUP;
use Lists::Utilities::StringFormator;
use Lists::Utilities::AmazonRequestSignatureHelper;
...
=head1 NAME
   ...
   Lists::Utilities::Amazon.pm

=head1 SYNOPSIS

   $Amazon = new Lists::Utilities::Amazon(Debugger, UserManager, DBManger);

=head1 DESCRIPTION

Used to communicate with Amazon and build affiliate links

=head2 Lists::Utilities::Amazon Constructor
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon constructor");

   if(! $self->{"AmazonToken"}){ 
      $self->{"AmazonToken"} = $Lists::SETUP::AMAZON_TOKEN;
   }
   if(! $self->{"AmazonSecret"}){
      $self->{"AmazonSecret"} = $Lists::SETUP::AMAZON_SECRET;
   }
   #roa2now-20
   if(! $self->{"AmazonUserName"}){ 
      $self->{"AmazonUserName"} = $Lists::SETUP::AMAZON_ASSOCIATE_TAG;
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon::getTopSearchResults with $query, $mode, $page");
   ...
   my $endpoint = $Lists::SETUP::AMAZON_REQUESTS{'ENDPOINT'}{'US'};
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon::getItemByASIN with $ASIN");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->getAmazonChoices");
   ...
   foreach my $ASIN(sort{$Choices->{$a}->{SortOrder} <=> $Choices->{$b}->{SortOrder}} keys %{$Choices}) {
      if($count < $Lists::SETUP::CONSTANTS{'AMAZON_CHOICE_LIMIT'}){
         ...
         $AmazonChoices .= $self->getBasicPage("$Lists::SETUP::DIR_PATH/Utilities/Amazon/choices_rows.html", \%Data);
         $count++;
      }
   }
...
#############################################
   ...
   my $AssociatesTag = $Lists::SETUP::AMAZON_REQUESTS{'ASSOCIATE_TAG'}{'US'};
   if($endpoint =~ m/\.ca$/){
      $AssociatesTag = $Lists::SETUP::AMAZON_REQUESTS{'ASSOCIATE_TAG'}{'CA'};
   }elsif($endpoint =~ m/\.co\.uk$/){
      $AssociatesTag = $Lists::SETUP::AMAZON_REQUESTS{'ASSOCIATE_TAG'}{'UK'};
   }
   ...
   my $helper = new Lists::Utilities::AmazonRequestSignatureHelper (
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon::sendAmazonRequest with $url");

   use XML::Simple;
   use LWP::UserAgent;

   my $ua = new LWP::UserAgent();
   $ua->timeout( $Lists::SETUP::CONSTANTS{'LWP_TIMEOUT'} );
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->runGeoLinkCheck");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->getAmazonLink with LinkID: $AmazonLinkID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->getAmazonImage with AmazonLinkID: $AmazonLinkID");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->setAmazonLink with ASIN: $ASIN, Country: $Country");

   my $AmazonItem = $self->getItemByASIN($ASIN, $Lists::SETUP::AMAZON_REQUESTS{'ENDPOINT'}{$Country});
   ...
   my $url = Lists::Utilities::StringFormator::htmlDecode($AmazonItem->{'URL'});
   my $imageurl = Lists::Utilities::StringFormator::htmlDecode($AmazonItem->{'LargeImage'});
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->getEndPoint : " . $Lists::SETUP::AMAZON_REQUESTS{'ENDPOINT'}{'US'});

   my $country = $self->getCountryByRemoteIP();

   my $endpoint = $Lists::SETUP::AMAZON_REQUESTS{'ENDPOINT'}{$country};
   if(! $endpoint){
      $endpoint = $Lists::SETUP::AMAZON_REQUESTS{'ENDPOINT'}{'US'};
   }
   return $endpoint;
}
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Utilities::Amazon->getCountryByRemoteIP $ENV{'REMOTE_ADDR'}");
