package Lists::Admin::Calculator;
use strict;
...
##########################################################
# Lists::Admin::Calculator 
##########################################################

=head1 NAME

   Lists::Admin::Calculator.pm

=head1 SYNOPSIS

   $ListCalculator = new Lists::Admin::Calculator($dbh, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::Calculator constructor");

   if(!$self->{DBManager}){
      die $self->{Debugger}->log("Where is Lists::Admin::Calculator's DBManager??");
   }
   if(!$self->{Debugger}){
...
=item B<Parameters :>
   ...
   1. $self - Reference to a Lists::Admin::Calculator object
...
#############################################
   ...
   my $TopReferrers = $ReferrerObj->getTopReferrers($Lists::SETUP::TOP_REFERRERS_TO_DISPLAY);
...
#############################################
   ...
   foreach my $daysBack(@Lists::SETUP::SUMMARY_STATS_DAYS_BACK) {
      my $DateClause = "";
      if($daysBack){
         my $lastMidnight = Lists::Utilities::Date::getTodayAtMidnight();
...
#############################################
   ...
   $self->getUserActivityInfo($UserWeek, $Lists::SETUP::RANKING_TIMEFRAMES{'LastWeek'}, "LastWeek");
   $self->getUserActivityInfo($UserYear, $Lists::SETUP::RANKING_TIMEFRAMES{'LastYear'}, "LastYear");
...
#############################################
   ...
   foreach my $field(keys %Lists::SETUP::RANKING_TIMEFRAMES) {
      my $Seconds = $Lists::SETUP::RANKING_TIMEFRAMES{$field};
...
#############################################
   ...
   $ListHash->{CreateDateFormatted} = Lists::Utilities::Date::getHumanFriendlyDate($ListHash->{CreateDate});
