package Lists::Admin::FeedbackManager;
use strict;
...
##########################################################
# Lists::Admin::FeedbackManager 
##########################################################

=head1 NAME

   Lists::Admin::FeedbackManager.pm

=head1 SYNOPSIS

   $ListManager = new Lists::Admin::FeedbackManager($dbh, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager constructor");
...
=item B<Parameters :>
   ...
   1. $self - Reference to a Lists::Admin::FeedbackManager object
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::getFeedbackElement with $element");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::getFeedbackRequestsHTML");

   my $html = $self->getFeedbackRequestsRows($FeedbackStatusID, $UserID, $FeedbackID);

   my $template = "$Lists::SETUP::ADMIN_DIR_PATH/reports/FeedbackReport.html";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::getFeedbackRequestsRows");
   ...
   foreach my $ID (sort{$FeedbackRequests->{$a}->{CreateDate} <=> $FeedbackRequests->{$b}->{CreateDate}} keys %{$FeedbackRequests}) {
      ...
      if($FeedbackRequests->{$ID}->{ProblematicListID}){
         ...
         $ProblematicList = qq~<a href="$Lists::SETUP::URL/$ListURL">$ProblematicListHash->{Name}</a>~;
      }   
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::getFeedbackRequests with FeedbackStatusID: $FeedbackStatusID, UserID: $UserID, FeedbackID: $FeedbackID");
   ...
   foreach my $ID(keys %{$FeedbackRequests}) {
	 ...
	 $FeedbackRequests->{$ID}->{User} = "guest";
      ...
      $FeedbackRequests->{$ID}->{CreateDateFormatted} = Lists::Utilities::Date::getHumanFriendlyDate($FeedbackRequests->{$ID}->{CreateDate});
   }
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::getFeedbackReplies with : $FeedbackID");
   ...
   foreach my $ID (sort keys %{$FeedbackReplies}) {
         ...
         my $Date = Lists::Utilities::Date::getHumanFriendlyDate($FeedbackReplies->{$ID}->{ReplyDate});;
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::addFeedbackReply");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::FeedbackManager::sendFeedbackReply");
   ...
   $Feedback->{"ReplyDate"} = Lists::Utilities::Date::getHumanFriendlyDate($Feedback->{CreateDate});
   ...
   my $EmailBodyHTML = $PageGetter->getBasicPage("$Lists::SETUP::DIR_PATH/emails/feedback_reply.html", \%Data);
   my $EmailBodyTXT = $PageGetter->getBasicPage("$Lists::SETUP::DIR_PATH/emails/feedback_reply.txt", \%Data);
   ...
   my $EmailTXT = $PageGetter->getBasicPage("$Lists::SETUP::DIR_PATH/emails/email_template.txt", \%Data);
   ...
   $Mailer->sendEmail($UserReporting->{Email}, 
                    $Lists::SETUP::MAIL_FROM_FEEDBACK, 
                    $subject, 
