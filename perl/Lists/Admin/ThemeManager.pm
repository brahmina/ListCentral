package Lists::Admin::ThemeManager;
use strict;
...
##########################################################
# Lists::Admin::ThemeManager 
##########################################################

=head1 NAME

   Lists::Admin::ThemeManager.pm

=head1 SYNOPSIS

   $ListManager = new Lists::Admin::ThemeManager($dbh, $debug);
...
########################################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager constructor");
...
=item B<Parameters :>
   ...
   1. $self - Reference to a Lists::Admin::ThemeManager object
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager::getThemeElement");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager::getAddNewThemeElements"); 
   ...
   foreach my $ID(sort{$a <=> $b} keys %{$ThemeColours}) {
      ...
      $ColourDivs .= qq~<div id="$div_name" class="ColourDiv">
                               <h3>$Lists::SETUP::THEME_POSITION{$i}</h3><br />
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager::getAvailableThemes");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->AddNewTheme");
   ...
   return "$Lists::SETUP::ADMIN_DIR_PATH/theme_management.html";
}
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->DisableTheme");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->EnableTheme");
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->generateCSS");
   ...
   # Create the new dirs
   my $themeDir = "$Lists::SETUP::BASE_CSS_DIR/$ThemeID";
   if(! -e $themeDir){
      my $output = `mkdir $themeDir`;
   }

   # Write the css files
   my $baseDir = "$Lists::SETUP::BASE_CSS_DIR/Base";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->refreshBaseCSSFiles");

   my $ThemeColoursObj = $self->{DBManager}->getTableObj("ThemeColours");
   my $ThemeColours = $ThemeColoursObj->get_with_restraints("ThemeID = $Lists::SETUP::CONSTANTS{'DEFAULT_THEME'}");
   my %OrderedThemeColours;
   ...
   # Create the new dirs
   my $defaultThemeDir = "$Lists::SETUP::DIR_PATH/css/$Lists::SETUP::CONSTANTS{'DEFAULT_THEME'}";
   my $baseThemeDir = "$Lists::SETUP::DIR_PATH/css/Base";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->generateImages");

   use Lists::Utilities::ImageColorConverter;
   my $ImageColorConverter = new Lists::Utilities::ImageColorConverter("Debugger" => $self->{Debugger});
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::ThemeManager->RegenerateThemes");
   ...
   foreach my $ThemeID(keys %{$Themes}) {
      if($ThemeID != $Lists::SETUP::CONSTANTS{'DEFAULT_THEME'}){
