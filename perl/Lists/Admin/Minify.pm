...
=head1 NAME
   ...
   Lists::Admin::Minify.pm

=head1 SYNOPSIS

   $Admin::Printer = new Lists::Admin::Minify($Debugger);

=head1 DESCRIPTION

Used to handle the page requests to the Lists web application

=head2 Lists::Admin::Minify Constructor
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::Minify->minifyJS");

   my $devDir = "$Lists::SETUP::DIR_PATH/js_src"; 
   # Where the development happens
   my $proDir = "$Lists::SETUP::DIR_PATH/js";
...
#############################################
   ...
   $self->{Debugger}->debug("in Lists::Admin::Minify->minifyJSDir with $source");
